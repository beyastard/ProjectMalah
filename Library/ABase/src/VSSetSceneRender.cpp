...
///////////////////////////////////////////////////////////////////////////
//
//	Implement of CVSSetSceneRender
//
///////////////////////////////////////////////////////////////////////////

CVSSetSceneRender::CVSSetSceneRender() : A3DScene()
{
	m_iNumRoute		= NULL;
	m_iCurRoute		= 0;
	m_bRenderAll	= false;
...
*/
bool CVSSetSceneRender::Init(A3DDevice* pA3DDevice)
{
	if (!A3DScene::Init(pA3DDevice))
	{
		g_pA3DErrLog->ErrLog("CVSSetSceneRender::Init, Failed to call A3DScene::Init.");
		return false;
	}
...
/*	Release resources
	...
	Return true for success, otherwise return false.
*/
bool CVSSetSceneRender::Release()
{
	RemoveAllRoutes();
...
/*	Set current route
	...
	iIndex: route's index returned by CVSSetSceneRender::LoadRoute().
	iSample: current sample of specified route
*/
bool CVSSetSceneRender::SetCurrentRoute(int iIndex, int iSample)
{
...
*/
bool CVSSetSceneRender::SetCurrentSection(int iSection)
{
...
/*	Update visible surface set
	...
	Return true for success, otherwise return false.
*/
bool CVSSetSceneRender::UpdateVisibleSets(A3DViewport* pCurrentViewport)
{
	if (!m_nNumTextures)
		return true;
...
*/
CVSSetSceneRender::ROUTE* CVSSetSceneRender::AllocRouteSlot()
{
	int i;
	...
	if (i >= MAXNUM_ROUTE)
	{
		g_pA3DErrLog->ErrLog("CVSSetSceneRender::AllocRouteSlot, too many routes.");
		return NULL;
	}
...
*/
int CVSSetSceneRender::LoadRoute(char* szFileName, bool bAll)
{
	int iNumFace, iNumObj;

	if (!(iNumFace = GetFaceCount()))
	{
		g_pA3DErrLog->ErrLog("CVSSetSceneRender::LoadRoute, scene data hasn't been loaded.");
		return -1;
	}
	...
	if (!pRoute)
	{
		g_pA3DErrLog->ErrLog("CVSSetSceneRender::Init, failed to allocate route slot!");
		return -1;
	}

	//	Create route object
	if (!(pRoute->pRoute = new CVSSetRouteLoader))
	{
		g_pA3DErrLog->ErrLog("CVSSetSceneRender::Init, failed to create route object!");
		return -1;
	}

	if (!(pRoute->pRoute->Load(szFileName, bAll)))
	{
		g_pA3DErrLog->ErrLog("CVSSetSceneRender::LoadRoute, failed to load route.");
		goto Error;
	}
	...
	{
		g_pA3DErrLog->ErrLog("CVSSetSceneRender::LoadRoute, not enough memory.");
		goto Error;
	}
...
*/
int	CVSSetSceneRender::LoadRoute(BYTE* pBuf)
{
	int iNumFace, iNumObj;
...
/*	Remove one route
	...
	iIndex: route's index returned by CVSSetSceneRender::LoadRoute().
*/
bool CVSSetSceneRender::RemoveRoute(int iIndex)
{
	if (iIndex < 0 || iIndex >= MAXNUM_ROUTE)
		return false;
...
//	Remove all routes
void CVSSetSceneRender::RemoveAllRoutes()
{
...
/*	Get route loader object
	...
	iIndex: route's index returned by CVSSetSceneRender::LoadRoute().
*/
CVSSetRouteLoader* CVSSetSceneRender::GetRouteLoader(int iIndex)
{
	if (iIndex < 0 || iIndex >= MAXNUM_ROUTE)
		return NULL;
...
*/
int	CVSSetSceneRender::GetSceneObjectNum()
{
	if (!m_iNumRoute)
		return 0;
...
/*	Get visible object's ID of specified section
	...
	pBuf (out): buffer used to receive visible object's ID, the buffer must be
				...
				CVSSetRouteLoader::GetObjectNum() to get the maximum possible 
...
*/
bool CVSSetSceneRender::GetCurrentVisObjs(int* pBuf, int* piNumVisObj)
{
