...
///////////////////////////////////////////////////////////////////////////
//
//	Implement of CVSSetRouteLoader
//
///////////////////////////////////////////////////////////////////////////

CVSSetRouteLoader::CVSSetRouteLoader()
{
	m_bReady		= false;
...
*/
bool CVSSetRouteLoader::Load(char* szFileName, bool bAll)
{
	if (m_bReady)
	{
		g_pA3DErrLog->ErrLog("CVSSetRouteLoader::Load, one route has been loaded.");
		return false;
	}

	if (!szFileName || !szFileName[0])
	{
		g_pA3DErrLog->ErrLog("CVSSetRouteLoader::Load, Invalid file name.");
		return false;
	}
	...
	if (!fp)
	{
		g_pA3DErrLog->ErrLog("CVSSetRouteLoader::Load, Failed to open file %s.", szFileName);
		return false;
	}
	...
	{
		fclose(fp);
		g_pA3DErrLog->ErrLog("CVSSetRouteLoader::Load, File version or format is wrong.");
		return false;
	}
	...
	{
		fclose(fp);
		g_pA3DErrLog->ErrLog("CVSSetRouteLoader::Load, Not enough memory for section information table.");
		return false;
	}
	...
	if (bAll)	//	Load all section vss data
	{
		...
		{
			g_pA3DErrLog->ErrLog("CVSSetRouteLoader::Load, Not enough memory for compressed vss data.");
			goto Error;
		}
...
*/
bool CVSSetRouteLoader::Load(AFile* pFile)
{
	return true;
}
...
/*	Load route vss from memory file.
	...
	pBuf: buffer which contain vss data. data format of pBuf should be the same
		  as vss file.
*/
bool CVSSetRouteLoader::Load(BYTE* pBuf)
{
	if (m_bReady)
	{
		g_pA3DErrLog->ErrLog("CVSSetRouteLoader::Load, one route has been loaded.");
	...
	{
		g_pA3DErrLog->ErrLog("CVSSetRoute::Load, File version or format is wrong.");
		return false;
	}
...
//	Release resources
void CVSSetRouteLoader::Release()
{
	if (m_bLoadAllOnce)
	{
		if (m_aCompVisBuf)
...
*/
int	CVSSetRouteLoader::GetSectionOfSample(int iSample)
{
	if (!m_bReady)
		return -1;
...
*/
bool CVSSetRouteLoader::GetSectionVisSurf(int iSection, int* pBuf, int* piNumVisSurf)
{
	if (!m_bReady)
		return false;
...
/*	Get visible object's ID of specified section
	...
	piNumVisObj (out): used to receive the number of object ID that can be seen 
					   ...
					   This parameter can be NULL
*/
bool CVSSetRouteLoader::GetSectionVisObjs(int iSection, int* pBuf, int* piNumVisObj)
{
#if VSSFILE_VERSION < 3
...
*/
bool CVSSetRouteLoader::DecodeVSS(BYTE* pSrc, int iSrcLen, int* pDest)
{
...
/*	Decompress section's VSS byte string.
	...
	pBuf: buffer used to store decompressed VSS string. This buffer must be
		  ...
		  CVSSetRouteLoader::GetVisByteNum().
*/
bool CVSSetRouteLoader::GetSectionVisBytes(int iSection, BYTE* pBuf)
{
	if (!m_bReady)
		return false;
