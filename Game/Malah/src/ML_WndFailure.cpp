...
///////////////////////////////////////////////////////////////////////////
//
//	Implement CMLWndFailure
//
///////////////////////////////////////////////////////////////////////////

IMPLEMENT_ASERIAL(CMLWndFailure, AWnd, 1)

CMLWndFailure::CMLWndFailure()
{
	m_bVisible		= false;
	m_bKeyPreview	= true;
	m_bCanBeFocus	= true;
...
bool CMLWndFailure::LoadFromIni(AWnd* pParent, AWndIniFile& IniFile, const char* pszSection)
{
	J_EXCEPTION_RECORD(0);
	...
	if (!m_btnRestart.LoadFromIni(this, IniFile, "wnd_failure::button_restart"))
	{
		ADEBUG_TRACE("cannot load replay button.");
		return false;
	}

	m_btnRestart.m_wnOnClick.SetValue(this, (AWndNotify::AMethodType)&CMLWndFailure::OnRestartClick);

	if (!m_btnExit.LoadFromIni(this, IniFile, "wnd_failure::button_exit"))
	{
		ADEBUG_TRACE("cannot load exit button.");
		return false;
	}

	m_btnExit.m_wnOnClick.SetValue(this, (AWndNotify::AMethodType)&CMLWndFailure::OnExitClick);

	return true;
}

bool CMLWndFailure::LoadRes()
{
	J_EXCEPTION_RECORD(0);
...
bool CMLWndFailure::OnRestartClick(AWnd* pWnd)
{
	J_EXCEPTION_RECORD(0);
...
bool CMLWndFailure::OnExitClick(AWnd* pWnd)
{
	J_EXCEPTION_RECORD(0);
...
bool CMLWndFailure::OnYesClick(AWnd* pWnd)
{
	J_EXCEPTION_RECORD(0);
...
bool CMLWndFailure::OnNoClick(AWnd* pWnd)
{
	J_EXCEPTION_RECORD(0);
...
bool CMLWndFailure::ShowWindow(bool bShow)
{
	J_EXCEPTION_RECORD(0);
...
void CMLWndFailure::SetYesNoMsgBox(CMLMessageBox* pMsgBox)
{
	if (!(m_pYesNoMsgBox = pMsgBox))
		return;

	m_pYesNoMsgBox->m_btnYes.m_wnOnClick.SetValue(this, (AWndNotify::AMethodType)&CMLWndFailure::OnYesClick);
	m_pYesNoMsgBox->m_btnNo.m_wnOnClick.SetValue(this, (AWndNotify::AMethodType)&CMLWndFailure::OnNoClick);
	m_pYesNoMsgBox->ShowWindow(false);
}
