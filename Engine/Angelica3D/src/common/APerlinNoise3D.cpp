...
bool APerlinNoise3D::Init(int nBufferWidth, int nBufferHeight, int nBufferDepth, float vAmplitude, int nWaveLength, float vPersistence, int nOctaveNum, DWORD dwRandSeed)
{
	...
	float * pValues = (float *) malloc(sizeof(float) * m_nBufferWidth * m_nBufferHeight * m_nBufferDepth);
	if( NULL == pValues )
		return false;

	m_pValues = (NOISEVALUE *) malloc(sizeof(NOISEVALUE) * m_nBufferWidth * m_nBufferHeight * m_nBufferDepth);
	if( NULL == m_pValues )
		return false;
