...
bool AM3DSoundBuffer::Create(PAMSoundEngine pAMSoundEngine, AMSBUFFERDESC * pDesc)
{
...
bool AM3DSoundBuffer::ReloadSoundData()
{
	AMS3DBUFFERDESC bufferDesc = m_3dbufferDesc;
	
	if( !AMSoundBuffer::ReloadSoundData() )
		return false;

	m_3dbufferDesc = bufferDesc;
	if( m_bForce2D )
	{
		m_bForce2D = false;
		SetForce2D(true);
...
bool AM3DSoundBuffer::Load(PAMSoundEngine pAMSoundEngine, AFile * pFileToLoad, char * szFileName)
{
...
AFileImage * AM3DSoundBuffer::FindSoundFileInMem()
{
	return m_pAMSoundEngine->Get3DSoundBufferMan()->Find3DSoundFileImage(m_szFileName);
}
