...
bool A3DLamp::Init(A3DDevice* pDevice, char* szShineTex, DWORD dwLampFlags)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DLamp::Init initlaize shine decal failed.");
		return false;
	}
...
void A3DLamp::SetLensFlareParams(char* szBaseName)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DLamp::SetLensFlareParams create lens flare object fail.");
		return;
	}

	if (!m_pLensFlare->Init(m_pA3DDevice, szBaseName))
	{
		g_pA3DErrLog->ErrLog("A3DLamp::SetLensFlareParams init lens flare fail.");
		return;
	}
}
...
void A3DLamp::Update(A3DViewport* pView)
{
	...
	if( m_pA3DDevice->GetA3DEngine()->GetEngineTicks() % 5 == 0 )
	{
		...
		if( pCDS )
		{
			if ((m_bObstruct = pCDS->RayTrace(vLampPos, vViewPos - vLampPos, 1.0f, &Trace, NULL)))
			{
				if (Trace.objectType != TRACE_OBJECT_MODEL ||
					Trace.pModel->GetVisibility())
				{
					// Be obstructed;
				}
				else
	...
	if (m_dwFlags & LAMPFLAG_DISTANCE)
	{
		...
		else if (fDist > m_fNearDist)
		{
			...
			m_fCurAlpha = min(1.0f, m_fCurSize + 0.5f);
			if (m_fCurAlpha == 0.0f)
				return; // Need not to show
		}
		else
