									  ...
									  float fDensity, float fViewField, float fStep)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DLightingEngine::BuildFogTable(), not enough memory");
		return false;
	}
...
void A3DLightingEngine::UpdateStaticLights()
{
	...
	while (pElem != pList->GetTail())
	{
		...
		if (m_iNumStaticLight >= MAXNUM_LIGHT)
		{
			...
			g_pA3DErrLog->ErrLog("A3DLightingEngine::UpdateStaticLights(), too many static lights");
			return;
		}
...
void A3DLightingEngine::SetDynamicLights(AList* pList)
{
	...
	while (pElem != pList->GetTail())
	{
		...
		{
			//	This shouldn't happen
			g_pA3DErrLog->ErrLog("A3DLightingEngine::SetDynamicLights(), too many lights");
			return;
		}
									...
									A3DMATERIALPARAM* pMaterial, A3DLVERTEX* aDestVerts)
{
	...
	for (i=0; i < iNumVert; i++)
	{
		...
		//	Clamp colors
		CLAMPMAXVALUE(Diffuse.r, 1.0f);
		CLAMPMAXVALUE(Diffuse.g, 1.0f);
		CLAMPMAXVALUE(Diffuse.b, 1.0f);
		CLAMPMAXVALUE(Diffuse.a, 1.0f);
		
		CLAMPMAXVALUE(Specular.r, 1.0f);
		CLAMPMAXVALUE(Specular.g, 1.0f);
		CLAMPMAXVALUE(Specular.b, 1.0f);
		CLAMPMAXVALUE(Specular.a, 1.0f);
