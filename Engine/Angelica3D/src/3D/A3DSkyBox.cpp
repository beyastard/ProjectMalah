...
bool A3DSkyBox::Init(A3DDevice * pA3DDevice, A3DCamera * pCamera, char * szRight, char * szLeft, char * szTop, char * szBottom, char * szFront, char * szBack)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DSkyBox::Init(), Can not load texture [%s]", szRight);
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DSkyBox::Init(), Can not load texture [%s]", szLeft);
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DSkyBox::Init(), Can not load texture [%s]", szTop);
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DSkyBox::Init(), Can not load texture [%s]", szBottom);
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DSkyBox::Init(), Can not load texture [%s]", szFront);
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DSkyBox::Init(), Can not load texture [%s]", szBack);
		return false;
	}
	...
	if( NULL == m_pSkyFace )
	{
		g_pA3DErrLog->ErrLog("A3DSkyBox::Init(), Not enough memory!");
		return false;
	}

	if( !m_pSkyFace->Init(m_pA3DDevice, A3DVT_LVERTEX, 4, 6, 0) )
	{
		g_pA3DErrLog->ErrLog("A3DSkyBox::Init(), Init the A3DStream fail!");
		return false;
	}
...
bool A3DSkyBox::Render()
{
	...
	for(int i=0; i<6; i++)
	{
		GetCubeMapDirAndUp(i, &vecDir, &vecUp);

		matWorld = TransformMatrix(vecDir, vecUp, A3DVECTOR3(0.0f)) * m_WorldMatrix;
...
bool A3DSkyBox::SetMapTexture(int nIndex, char * szMapTexture)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DSkyBox::Init(), Can not load texture [%s]", szMapTexture);
		return false;
	}
