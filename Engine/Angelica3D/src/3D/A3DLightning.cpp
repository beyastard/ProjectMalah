...
bool A3DLightning::Init(A3DDevice * pDevice, FLOAT vStrikeSize, FLOAT vStepSize, int nSteps, int nMaxLevel/*=4*/, A3DCOLOR color/*=0xffffffff*/)
{
	...
	m_ppFlashes = (A3DFlash **) malloc(sizeof(A3DFlash *) * m_nNumFlashes);
	if( NULL == m_ppFlashes )
	{
		g_pA3DErrLog->ErrLog("A3DLightning::Init(), Not enough memory!");
		return false;
	}
...
bool A3DLightning::AddPath(const A3DMATRIX4& matOrient, const A3DVECTOR3& vecStart, const A3DVECTOR3 &vecDir, int nNoisePos, int nLevel, int& nNumLeft)
{
	...
	vecEnd = vecStart + vecDir * (A3DRandGenerator::RandUniformFloat(0.5f, 1.5f) * m_vStepSize);
	...
	if( nLevel <= m_nMaxLevel && A3DRandGenerator::RandUniformFloat(0.0f, 1.0f) < 0.5f )
	{
		int nNum1;
		nNum1 = int(nNumLeft * A3DRandGenerator::RandUniformFloat(0.2f, 0.8f));
		...
		if( nNum1 )
		{
			...
			vecNewDir.z = g_pA3DMathUtility->COS(vPitch);
			vecNewDir.x = g_pA3DMathUtility->SIN(vPitch) * g_pA3DMathUtility->COS(vDeg);
			vecNewDir.y = g_pA3DMathUtility->SIN(vPitch) * g_pA3DMathUtility->SIN(vDeg);
			...
			A3DMATRIX4 matNewOrient = TransformMatrix(vecNewDir, vecUp, A3DVECTOR3(0.0f));
			AddPath(matNewOrient, vecEnd, vecNewDir, nNoisePos + A3DRandGenerator::RandUniformInt() % 1023, nLevel + 1, nNum1);
		}
	}
...
bool A3DLightning::Build()
{
	...
	m_matOrient = TransformMatrix(m_vecDir, vecUp, A3DVECTOR3(0.0f));

	// Build extire strike path
	if( !AddPath(m_matOrient, m_vecStart, m_vecDir, A3DRandGenerator::RandUniformInt() % 1023, 0, nTotalCount) )
		return true;

	A3DCamera * pCamera = m_pA3DDevice->GetA3DEngine()->GetActiveCamera();
