...
bool A3DLSubMesh::AddVerts(LPVOID pVertices, int nVertCount, A3DVERTEXTYPE vertexType)
{
	...
	if( NULL == pVB )
	{
		g_pA3DErrLog->ErrLog("A3DLSubMesh::AddVerts(), Not enough memory!");
		return false;
	}
...
bool A3DLSubMesh::AddFaces(LPVOID pIndices, int nFaceCount, A3DFORMAT indexFormat)
{
	...
	if( NULL == pIB )
	{
		g_pA3DErrLog->ErrLog("A3DLSubMesh::AddFaces(), Not enough memory!");
		return false;
	}
...
bool A3DIBLSubMesh::SetLightMap(int nLMWidth, int nLMHeight, LPBYTE pLMImage)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DIBLSubMesh::AddIBLFace(), Create light map fail!");
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DIBLSubMesh::AddIBLFace(), Lock light map fail!");
		return false;
	}
...
bool A3DIBLSubMesh::LoadLightMap()
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DIBLSubMesh::LoadLightMap(), fail to load lightmap texture [%s]!", m_szLightMapPath);
		return false;
	}
...
bool A3DIBLSubMesh::AddVerts(LPVOID pVertices, int nVertCount, A3DVERTEXTYPE vertexType)
{
	return true;
}

bool A3DIBLSubMesh::AddFaces(LPVOID pIndices, int nIndexCount, A3DFORMAT indexFormat)
{
	return true;
}
...
bool A3DIBLMesh::Init(A3DDevice * pDevice, char * pszBaseTexture, A3DIBLLIGHTRANGE iblLightRange, bool bUseLightMaps)
{
	...
	if( pszBaseTexture )
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DIBLMesh::Init(), fail to load base texture [%s]!", pszBaseTexture);
			return false;
		}
	}
...
bool A3DIBLMesh::AddIBLTriSubMesh(A3DIBLTriSubMesh ** ppNewIBLTriSubMesh)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DIBLMesh::AddIBLTriSubMesh(), call new sub-mesh's Init() fail!");
		return false;
	}
...
bool A3DIBLMesh::AddIBLBBSubMesh(A3DIBLBBSubMesh ** ppNewIBLBBSubMesh)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DIBLMesh::AddIBLBBSubMesh(), call new sub-mesh's Init() fail!");
		return false;
	}
...
bool A3DIBLMesh::AddIBLPlaneSubMesh(A3DIBLPlaneSubMesh ** ppNewIBLPlaneSubMesh)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DIBLMesh::AddIBLPlaneSubMesh(), call new sub-mesh's Init() fail!");
		return false;
	}
...
bool A3DIBLMesh::AddLSubMesh(A3DLSubMesh ** ppNewLSubMesh)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DIBLMesh::AddLSubMesh(), call new sub-mesh's Init() fail!");
		return false;
	}
