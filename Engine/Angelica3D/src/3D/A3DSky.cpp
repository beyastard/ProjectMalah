...
bool A3DSky::Init(A3DDevice * pDevice, A3DCamera * pCamera, char * szCap, char * szSurroundF, char * szSurroundB, char * szFlat)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DSky::Init Not enough memory!");
		return false;
	}
	...
	if( !bval )
	{
		g_pA3DErrLog->ErrLog("A3DSky::Init Stream Surround Init Fail!");
		return false;
	}
	bval = m_pStreamSurround->SetVerts((LPBYTE)m_pVBSurround, m_nVertCountS);
	if( !bval )
	{
		g_pA3DErrLog->ErrLog("A3DSky::Init Stream Surround Set Verts Fail!");
		return false;
	}
	bval = m_pStreamSurround->SetIndices(m_pIndicesSurround, m_nIndexCountS);
	if( !bval )
	{
		g_pA3DErrLog->ErrLog("A3DSky::Init Stream Surround Set Indices Fail!");
		return false;
	}
...
#define PULL_DOWN 0.7f
	...
	bval = m_pStreamFlat->Init(m_pA3DDevice, A3DVT_LVERTEX, m_nVertCountF, m_nIndexCountF, 0, true, true);
	if( !bval )
	{
		g_pA3DErrLog->ErrLog("A3DSky::Init Stream Flat Init Fail!");
		return false;
	}
	bval = m_pStreamFlat->SetVerts((LPBYTE)m_pVBFlat, m_nVertCountF);
	if( !bval )
	{
		g_pA3DErrLog->ErrLog("A3DSky::Init Stream Flat Set Verts Fail!");
		return false;
	}
	bval = m_pStreamFlat->SetIndices(m_pIndicesFlat, m_nIndexCountF);
	if( !bval )
	{
		g_pA3DErrLog->ErrLog("A3DSky::Init Stream Flat Set Indices Fail!");
		return false;
	}
	...
	bval = m_pStreamCap->Init(m_pA3DDevice, A3DVT_LVERTEX, m_nVertCountC, m_nIndexCountC, 0);
	if( !bval )
	{
		g_pA3DErrLog->ErrLog("A3DSky::Init Stream Cap Init Fail!");
		return false;
	}
	bval = m_pStreamCap->SetVerts((LPBYTE)m_pVBCap, m_nVertCountC);
	if( !bval )
	{
		g_pA3DErrLog->ErrLog("A3DSky::Init Stream Cap Set Verts Fail!");
		return false;
	}
...
bool A3DSky::TickAnimation()
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DSky::Render Stream Flat Set Verts Fail!");
		return false;
	}
...
bool A3DSky::SetCamera(A3DCamera * pCamera)
{
	if( !m_pA3DDevice ) return true;
...
bool A3DSky::SetTextureSurroundF(char * szTextureFile)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DSky::SetTextureSurroundF(), Can't load texture [%s]", szTextureFile);
		return false;
	}
...
bool A3DSky::SetTextureSurroundB(char * szTextureFile)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DSky::SetTextureSurroundB(), Can't load texture [%s]", szTextureFile);
		return false;
	}
