...
bool A3DGDI::Init(A3DDevice * pA3DDevice, char * pszTextureMap)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DGDI::Init() Not enough memory!");
		return false;
	}
	if( !m_pA3DStream->Init(m_pA3DDevice, A3DVT_LVERTEX, A3DGDI_MAX_FACES * 3, A3DGDI_MAX_FACES * 3, 0, true, true) )
	{
		g_pA3DErrLog->ErrLog("A3DGDI::Init() Init Stream Fail!");
		return false;
	}
	m_pA3DTLStream = new A3DStream();
	if( NULL == m_pA3DTLStream )
	{
		g_pA3DErrLog->ErrLog("A3DGDI::Init() Not enough memory!");
		return false;
	}
	if( !m_pA3DTLStream->Init(m_pA3DDevice, A3DVT_TLVERTEX, A3DGDI_MAX_FACES * 3, A3DGDI_MAX_FACES * 3, 0, true, true) )
	{
		g_pA3DErrLog->ErrLog("A3DGDI::Init() Init TL Stream Fail!");
		return false;
	}
...
bool A3DGDI::DrawTriangle(const A3DVECTOR3& vecPos, const A3DVECTOR3& vecDir, const A3DVECTOR3& vecUp, FLOAT vSize1, FLOAT vSize2, A3DCOLOR color)
{
	...
	matTM = TransformMatrix(vecDir, vecTrueUp, vecPos);
	...
	m_pA3DStream->SetVerts((BYTE *)m_pVertexBuffer, 3);
	m_pA3DStream->SetIndices(m_pIndexBuffer, 6);
	m_pA3DStream->Appear();
	...
	{
		g_pA3DErrLog->ErrLog("A3DGDI::DrawTriangle() DrawIndexedPrimitive Fail!");
		return false;
	}
	if( m_pA3DTexture )
...
bool A3DGDI::DrawRectangle(const A3DVECTOR3& vecPos, const A3DVECTOR3& vecDir, const A3DVECTOR3& vecUp, FLOAT vSize1, FLOAT vSize2, A3DCOLOR color, bool b2Sided)
{
	...
	m_pA3DStream->SetVerts((BYTE *)m_pVertexBuffer, 4);
	m_pA3DStream->SetIndices(m_pIndexBuffer, 12);
	m_pA3DStream->Appear();
	if( m_pA3DTexture )
		m_pA3DTexture->Appear(0);
	else
...
bool A3DGDI::Draw2DRectangle(const A3DRECT& rect, A3DCOLOR color)
{
	...
	m_pA3DTLStream->SetVerts((BYTE *)m_pTLVertexBuffer, 4);
	m_pA3DTLStream->SetIndices(m_pIndexBuffer, 6);
...
bool A3DGDI::Draw2DLine(const A3DPOINT2& ptStart, const A3DPOINT2& ptEnd, A3DCOLOR color)
{
	...
	m_pA3DTLStream->SetVerts((BYTE *)m_pTLVertexBuffer, 2);
	m_pA3DTLStream->SetIndices(m_pIndexBuffer, 2);
	m_pA3DTLStream->Appear();
	if( m_pA3DTexture )
		m_pA3DTexture->Appear(0);
	else
...
bool A3DGDI::Draw3DLine(const A3DVECTOR3& vecStart, const A3DVECTOR3& vecEnd, A3DCOLOR color)
{
	...
	m_pA3DStream->SetVerts((BYTE *)m_pVertexBuffer, 2);
	m_pA3DStream->SetIndices(m_pIndexBuffer, 2);
	m_pA3DStream->Appear();
	if( m_pA3DTexture )
		m_pA3DTexture->Appear(0);
	else
...
bool A3DGDI::Draw2DPoint(const A3DPOINT2& ptPos, A3DCOLOR color, FLOAT vSize)
{
	...
	m_pA3DTLStream->SetVerts((BYTE *)m_pTLVertexBuffer, 1);
	m_pA3DTLStream->SetIndices(m_pIndexBuffer, 1);
	m_pA3DTLStream->Appear();
	if( m_pA3DTexture )
		m_pA3DTexture->Appear(0);
	else
	...
	if( !m_pA3DDevice->DrawIndexedPrimitive(A3DPT_POINTLIST, 0, 1, 0, 1) )
	{
		g_pA3DErrLog->ErrLog("A3DGDI::DrawTriangle() DrawIndexedPrimitive Fail!");
		return false;
	}
...
bool A3DGDI::Draw3DPoint(const A3DVECTOR3& vecPos, A3DCOLOR color, FLOAT vSize)
{
	...
	m_pA3DStream->SetVerts((BYTE *)m_pVertexBuffer, 1);
	m_pA3DStream->SetIndices(m_pIndexBuffer, 1);
	m_pA3DStream->Appear();
	if( m_pA3DTexture )
		m_pA3DTexture->Appear(0);
	else
...
bool A3DGDI::Draw2DTriangle(const A3DPOINT2& v0, const A3DPOINT2& v1, const A3DPOINT2& v2, A3DCOLOR color)
{
	...
	m_pA3DTLStream->SetVerts((BYTE *)m_pTLVertexBuffer, 3);
	m_pA3DTLStream->SetIndices(m_pIndexBuffer, 6);
	m_pA3DTLStream->Appear();
	m_pA3DDevice->SetZWriteEnable(false);
	m_pA3DDevice->SetZTestEnable(false);
