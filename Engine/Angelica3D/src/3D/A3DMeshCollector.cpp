...
A3DMeshCollector::A3DMeshCollector() : m_aMeshPool(100)
{
	m_pA3DDevice	= NULL;
	m_iPoolCnt		= 0;
}
...
bool A3DMeshCollector::Init(A3DDevice* pA3DDevice)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DMeshCollector::Init failed to init texture list.");
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DMeshCollector::Init, not enough memory!");
		return false;
	}
...
DWORD A3DMeshCollector::RegisterTexture(A3DTexture* pTexture, A3DMaterial* pMaterial)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DMeshCollector::RegisterTexture not enough memory");
		return 0;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DMeshCollector::RegisterTexture failed to create texture");
		return 0;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DMeshCollector::RegisterTexture failed to create material");
		return 0;
	}
										   ...
										   DWORD hTexture)
{
	...
	if (pSlot->iMeshCnt == pSlot->aMeshInfo.size())
		pSlot->aMeshInfo.push_back(m_aMeshPool[m_iPoolCnt-1]);
	else
			...
			for (i=0; i < iNumMesh; i++)
			{
					...
					iNumIndex + iIdxInMesh > MAXNUM_RENDERIDX)
				{
					if (iVertInMesh > MAXNUM_RENDERVERT || iIdxInMesh > MAXNUM_RENDERIDX)
					{
						...
						g_pA3DErrLog->ErrLog("A3DMeshCollector::Render, too much vertex in mesh!");
						return false;
					}
					...
					{
						g_pA3DErrLog->ErrLog("A3DMeshCollector::Render, render failed!");
						return false;
					}
