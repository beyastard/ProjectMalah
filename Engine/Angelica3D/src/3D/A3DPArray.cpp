...
{
	m_pA3DDevice	= NULL;
	m_dwClassID		= A3D_CLASS_ID_PARRAY;	
...
bool A3DPArray::CreateObjectFragment(A3DModel* pObjectModel)
{
	...
	if (!m_aFragVerts)
	{
		g_pA3DErrLog->ErrLog("A3DPArray::InitObjectFragment Not enough memory");
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DPArray::InitObjectFragment call parent CreateObjectFragment fail!");
		return false;
	}
...
bool A3DPArray::AddMeshInfo(A3DFrame* pFrame, A3DMesh* pMesh, A3DTexture* pTexture)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DPArray::AddMeshInfo(), Too much mesh or texture");
		return false;
	}
...
void A3DPArray::SplitModel()
{
	...
	for (i=0; i < m_iNumMesh; i++)
	{
		...
		if (!(aVerts = (PA3DLVERTEX)malloc(iNumVert * sizeof (A3DLVERTEX))))
		{
			g_pA3DErrLog->ErrLog("A3DPArray::SplitModel(), Not enough memory");
			return;
		}
...
bool A3DPArray::SplitTriangle(A3DLVERTEX* aVerts, A3DVECTOR3 vNormal, float fAreaVal)
{
	...
	float fArea = fAreaVal >= 0.0f ? fAreaVal : Magnitude(CrossProduct(d1, d2)) * 0.5f;
...
bool A3DPArray::SplitQuadrangle(A3DLVERTEX* aVerts, A3DVECTOR3 vNormal, float fAreaVal)
{
	...
	if (fAreaVal < 0.0f)
	{
		...
		fArea1	= Magnitude(CrossProduct(d1, d2)) * 0.5f;
		fArea2	= Magnitude(CrossProduct(d2, d3)) * 0.5f;
		fArea	= fArea1 + fArea2;
	}
	else
		fArea = fAreaVal;
...
bool A3DPArray::EmitOneFragment(A3DLVERTEX* aVerts, int iNumVert, A3DVECTOR3 vNormal)
{
	...
	if (iNumVert == 3)	//	Is a triangle
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DPArray::EmitOneFragment() buffer overflow!");
			return false;
		}
	...
	A3DMATRIX4 matRotateVar = RotateX(RandFloat(0, 30.0f)) * RotateY(RandFloat(0, 30.0f));
	...
	if( (A3DRandGenerator::RandUniformInt() % 100) < 100 && m_iNumGFXs < 1 )
	{
...
bool A3DPArray::EmitParticles()
{
	...
	for(int n=0; n<nToNew; n++)
	{
		...
		case A3DPARTICLE_META_PARTICLES:
			...
			g_pA3DErrLog->ErrLog("A3DPArray::EmitParticles(), Not implemented now!");
			return false;
...
bool A3DPArray::MakeDead(LPVOID pParticle)
{
	...
	case A3DPARTICLE_META_PARTICLES:
		g_pA3DErrLog->ErrLog("A3DPArray::MakeDead(), Not implemented now!");
		return false;
