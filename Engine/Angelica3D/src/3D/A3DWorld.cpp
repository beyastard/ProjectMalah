...
A3DWorld::A3DWorld()
{
	...
	ZeroMemory(m_pA3DStars, sizeof(A3DStars *) * A3DWORLD_MAX_STARSYSTEM);
...
bool A3DWorld::Init(A3DDevice * pA3DDevice, char * szWorldFile)
{
	A3DVECTOR3 vecPos, vecDir, vecUp;

	strcpy(m_szFolderName, "Scenes");

	m_pA3DDevice = pA3DDevice;
	strncpy(m_szWorldFile, szWorldFile, MAX_PATH);

	m_pA3DDevice->GetA3DEngine()->ResetWorldRelated();
	...
	{
		g_pA3DErrLog->ErrLog("A3DWorld::Init Can not locate the file: %s", szWorldFile);
		return false;
	}
	if( theFile.IsBinary() )
	{
	...
	else
	{
		...
		theFile.GetStringAfter(szLineBuffer, "WORLD: ", szResult);
		SetName(szResult);

		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		if( strcmp(szLineBuffer, "{") )
		{
			g_pA3DErrLog->ErrLog("A3DWorld::Init File parse error: %s", szWorldFile);
			return false;
		}

		//Load Terrain Parameter First;
		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		theFile.GetStringAfter(szLineBuffer, "TERRAINTEXTURE: ", szResult);
		strncpy(m_worldConfig.szTexBase, szResult, MAX_PATH);
		
		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		theFile.GetStringAfter(szLineBuffer, "TERRAINTEXTURENUM: ", szResult);
		m_worldConfig.terrainParam.nNumTexture = atoi(szResult);		

		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		theFile.GetStringAfter(szLineBuffer, "TERRAINWIDTH: ", szResult);
		m_worldConfig.terrainParam.nWidth = atoi(szResult);		

		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		theFile.GetStringAfter(szLineBuffer, "TERRAINHEIGHT: ", szResult);
		m_worldConfig.terrainParam.nHeight = atoi(szResult);		

		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		theFile.GetStringAfter(szLineBuffer, "TERRAINSIGHTRANGE: ", szResult);
		m_worldConfig.terrainParam.nSightRange = atoi(szResult);		

		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		theFile.GetStringAfter(szLineBuffer, "TERRAINTEXTURECOVER: ", szResult);
		m_worldConfig.terrainParam.nTextureCover = atoi(szResult);		

		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		theFile.GetStringAfter(szLineBuffer, "TERRAINCELLSIZE: ", szResult);
		m_worldConfig.terrainParam.vCellSize = (FLOAT) atof(szResult);		

		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		theFile.GetStringAfter(szLineBuffer, "TERRAINOFFSET: ", szResult);
		m_worldConfig.terrainParam.vOffset = (FLOAT) atof(szResult);		

		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		theFile.GetStringAfter(szLineBuffer, "TERRAINSCALE: ", szResult);
		m_worldConfig.terrainParam.vScale = (FLOAT) atof(szResult);		

		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		theFile.GetStringAfter(szLineBuffer, "TERRAINHEIGHTMAP: ", szResult);
		strncpy(m_worldConfig.szHeightMap, szResult, MAX_PATH);

		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		theFile.GetStringAfter(szLineBuffer, "TERRAINTEXTUREMAP: ", szResult);
		strncpy(m_worldConfig.szTexMap, szResult, MAX_PATH);
		
		//Then load Sky Parameter;
		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		theFile.GetStringAfter(szLineBuffer, "SKYCAP: ", szResult);
		strncpy(m_worldConfig.szSkyCap, szResult, MAX_PATH);

		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		theFile.GetStringAfter(szLineBuffer, "SKYSURROUNDF: ", szResult);
		strncpy(m_worldConfig.szSkySurroundF, szResult, MAX_PATH);

		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		theFile.GetStringAfter(szLineBuffer, "SKYSURROUNDB: ", szResult);
		strncpy(m_worldConfig.szSkySurroundB, szResult, MAX_PATH);

		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		theFile.GetStringAfter(szLineBuffer, "SKYFLAT: ", szResult);
		strncpy(m_worldConfig.szSkyFlat, szResult, MAX_PATH);

		//Last, load esp file name;
		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		theFile.GetStringAfter(szLineBuffer, "ESPFILE: ", szResult);
		strncpy(m_worldConfig.szESPFile, szResult, MAX_PATH);

		//Now Load building frames;
		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		theFile.GetStringAfter(szLineBuffer, "BUILDINGMODEL: ", szResult);
		...
		if( nBuildingModelCount )
		{
			...
			for(i=0; i<nBuildingModelCount; i++)
			{
				...
				theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
				if( !m_pA3DDevice->GetA3DEngine()->GetA3DModelMan()->LoadModelFile(szLineBuffer, &pBuildingModel) )
				{
					g_pA3DErrLog->ErrLog("A3DWorld::Init Can't load building model file: %s", szLineBuffer);
					return false;
				}
				theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		...
		theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		theFile.GetStringAfter(szLineBuffer, "OBJECTMODEL: ", szResult);
		...
		if( nObjectModelCount )
		{
			...
			for(i=0; i<nObjectModelCount; i++)
			{
				...
				theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
				if( !m_pA3DDevice->GetA3DEngine()->GetA3DModelMan()->LoadModelFile(szLineBuffer, &pObjectModel) )
				{
					g_pA3DErrLog->ErrLog("A3DWorld::Load Can't load building model file: %s", szLineBuffer);
					return false;
				}
				theFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
				sscanf(szLineBuffer, "(%f, %f, %f)", &vecPos.x, &vecPos.y, &vecPos.z);
	...
	{
		g_pA3DErrLog->ErrLog("A3DWorld::Init Not enough memory!");
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DWorld::Init Init A3DTerrain Fail!");
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DWorld::Init Init A3DSky Fail!");
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DWorld::Init(), Error init A3DScene!");
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DWorld::Init Load ESP File Fail!");
		return false;
	}
	...
	if (m_pSun)
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DWorld::Init Init sum lamp fail!");
			delete m_pSun;
			m_pSun = NULL;
		}
	}
	...
	*/
	...
	m_pA3DDevice->GetA3DEngine()->SetA3DCDS(m_pA3DWorldCDS);
	m_pA3DDevice->GetA3DEngine()->SetTerrain(m_pA3DTerrain);
	m_pA3DDevice->GetA3DEngine()->SetSky(m_pA3DSky);
	m_pA3DDevice->GetA3DEngine()->SetWorld(this);
	return true;
}

bool A3DWorld::Create(A3DDevice * pA3DDevice, bool bCreateTerrain, bool bCreateSky)
{
	A3DVECTOR3 vecPos, vecDir, vecUp;
...
bool A3DWorld::Init(A3DDevice * pA3DDevice, A3DWORLD_CONFIG& worldConfig)
{
...
bool A3DWorld::Release()
{
	...
	for(int i=0; i<A3DWORLD_MAX_STARSYSTEM; i++)
	{
		if( m_pA3DStars[i] )
		{
	...
	while( pThisBuildingModelElement != m_ListBuildingModels.GetTail() )
	{
		...
		m_pA3DDevice->GetA3DEngine()->GetA3DModelMan()->ReleaseModel(pA3DModel);
	...
	m_pA3DDevice->GetA3DEngine()->SetA3DCDS(NULL);
	m_pA3DDevice->GetA3DEngine()->SetSky(NULL);
	m_pA3DDevice->GetA3DEngine()->SetTerrain(NULL);
	m_pA3DDevice->GetA3DEngine()->SetWorld(NULL);
...
bool A3DWorld::Render(A3DViewport * pCurrentViewport)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DEngine::RenderScene Render Sky Fail!");
		return false;
	}
	...
	for(int i=0; i<A3DWORLD_MAX_STARSYSTEM; i++)
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DEngine::RenderScene(), Render Stars Fail!");
			return false;
		}
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DEngine::RenderScene Render Terrain Fail!");
		return false;
	}

	//m_pA3DDevice->SetSpecularEnable(true);
	m_pA3DDevice->GetA3DEngine()->SetBuildingRenderFlag(false);
	...
	while( pThisObjectModelElement != m_ListObjectModels.GetTail() )
	{
		...
		if( max(max(vecExt.x, vecExt.y), vecExt.z) < 50.0f )
		{
			if( !m_pA3DScene->IsVisiblePos(pA3DModel->GetModelAABB().Center) )
				goto NEXT;
		}
...
bool A3DWorld::GetFirstCollision(A3DModel * pModel, A3DVECTOR3 vecDelta, A3DWORLD_COLLISION * pCollision)
{
	...
	while( pObjectElement != m_ListObjectModels.GetTail() )
	{
		...
		if( CLS_AABBToAABB(aabbSource.Center, aabbSource.Extents, aabbTarget.Center, aabbTarget.Extents) )
		{
	...
	*pVecClipDelta = vecDelta;
	...
	for(int i=0; i<3; i++)
	{
		...
		else
		{
			...
			//Intersection box;
			vIntersection = min(maxs1.m[i], maxs2.m[i]) - max(mins1.m[i], mins2.m[i]);
...
bool A3DWorld::RayTrace(A3DVECTOR3& vecStart, A3DVECTOR3& vecVelocity, FLOAT vTime, RAYTRACE * pRayTrace, A3DModel * pModelMe)
{
	RAYTRACE	rayTrace;
	
	m_pA3DDevice->GetA3DEngine()->BeginPerformanceRecord(A3DENGINE_PERFORMANCE_WORLD_RAYTRACE);
	...
	if( m_pA3DDevice->GetA3DEngine()->GetA3DPlants() && m_pA3DDevice->GetA3DEngine()->GetA3DPlants()->RayTrace(vecStart, vecVelocity * vTime, &rayTrace) )
	{
		//If it is nearer;
		if( rayTrace.fFraction < pRayTrace->fFraction )
			*pRayTrace = rayTrace;
...
Next:
	...
	m_pA3DDevice->GetA3DEngine()->EndPerformanceRecord(A3DENGINE_PERFORMANCE_WORLD_RAYTRACE);
...
bool A3DWorld::OBBTrace(A3DVECTOR3& vecStart, A3DVECTOR3& vecVelocity, FLOAT vTime, OBBSHAPE& obbShape, OBBTRACE * pOBBTrace, A3DModel * pModelMe, bool bFirstTime)
{
	A3DOBB		obb;
	OBBTRACE	obbTrace;
	
	TRA_OBBTraceInit(&obb, vecStart, vecVelocity, vTime, obbShape);

	pOBBTrace->fFraction = 1.0f;
	
	ZeroMemory(&obbTrace, sizeof(obbTrace));
	
	//First test collision with terrain;
	if( m_pA3DTerrain && m_pA3DTerrain->OBBTrace(&obbTrace, obb, obbShape) )
	{
		//First collision, record it;
		*pOBBTrace = obbTrace;
	}

	//	There isn't OBBTrace any longer ...
	//Now test if the OBB intersect with buildings;
//	if( m_pA3DESP && m_pA3DESP->OBBTrace(&obbTrace, obb, obbShape) )
//	{
...
//	}
	...
	while( pObjectElement != m_ListObjectModels.GetTail() )
	{
		...
		if( pModel->OBBTrace(obb, obbShape, &obbTrace, m_dwModelAABBTraceMask) )
		{
			if( obbTrace.fFraction < pOBBTrace->fFraction )
			{
				//This is nearer
...
Next:
	...
	pOBBTrace->vPoint = vecStart + vecVelocity * vTime;
	return false;
}		

bool A3DWorld::AABBTrace(A3DVECTOR3& vStart, A3DVECTOR3& vExts, A3DVECTOR3& vVelocity, 
						 FLOAT fTime, AABBTRACE* pTrace, A3DModel *pModelMe)
{
	AABBTRACEINFO Info;
	AABBTRACE Trace;
	bool bHit = false;

	m_pA3DDevice->GetA3DEngine()->BeginPerformanceRecord(A3DENGINE_PERFORMANCE_WORLD_AABBTRACE);
...
*/
	...
	if (m_pA3DDevice->GetA3DEngine()->GetA3DPlants() && m_pA3DDevice->GetA3DEngine()->GetA3DPlants()->AABBTrace(&Info, &Trace))
	{
		bHit = true;
...
Next:
	...
	m_pA3DDevice->GetA3DEngine()->EndPerformanceRecord(A3DENGINE_PERFORMANCE_WORLD_AABBTRACE);
...
{
	if( index >= A3DWORLD_MAX_STARSYSTEM ) return false;
