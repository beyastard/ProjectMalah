...
A3DBillBoard::A3DBillBoard()
{
	...
	m_nDLightID		= -1;
	m_dwClassID = A3D_CLASS_ID_BILLBOARD;
}
...
bool A3DBillBoard::Create()
{
	...
	if( strlen(m_szTextureMap) )
	{
		if( !m_pA3DDevice->GetA3DEngine()->GetA3DTextureMan()->LoadTextureFromFileInFolder(m_szTextureMap, m_pA3DDevice->GetA3DEngine()->GetA3DGFXMan()->GetTextureFolder(), &m_pTexture, A3DTF_MIPLEVEL, 1) )
		{
			g_pA3DErrLog->ErrLog("A3DBillBoard::Create(), Can not find texture: [%s]", m_szTextureMap);
			return false;
		}
	}
	...
	if( NULL == m_pStream )
	{
		g_pA3DErrLog->ErrLog("A3DBillBoard::Create() Not enough memory!");
		return false;
	}

	A3DCDS * pA3DCDS = m_pA3DDevice->GetA3DEngine()->GetA3DCDS();
	if( (m_Type == A3DBILLBOARD_MARK || m_Type == A3DBILLBOARD_DYNAMICLIGHT || m_Type == A3DBILLBOARD_SHADOW) && pA3DCDS && pA3DCDS->GetA3DESP() )
	{
		m_pMarkVerts = (A3DLVERTEX *) malloc(sizeof(A3DLVERTEX) * A3DESP::MAXNUM_MARKVERT);
		if( NULL == m_pMarkVerts )
		{
			g_pA3DErrLog->ErrLog("A3DBillBoard::Create() Not enough memory!");
			return false;
		}
		m_pMarkIndices = (WORD *) malloc(sizeof(WORD) * A3DESP::MAXNUM_MARKINDEX);
		...
		if( !m_pStream->Init(m_pA3DDevice, A3DVT_LVERTEX, A3DESP::MAXNUM_MARKVERT, A3DESP::MAXNUM_MARKINDEX, 0, true, true) )
		{
			g_pA3DErrLog->ErrLog("A3DBillBoard::Create() Init A3DStream Fail!");
			return false;
		}
	}
	else
	{
		if( !m_pStream->Init(m_pA3DDevice, A3DVT_LVERTEX, 12, 18, 0) )
		{
			g_pA3DErrLog->ErrLog("A3DBillBoard::Create() Init A3DStream Fail!");
			return false;
		}
	...
	if( m_pA3DDevice->GetA3DEngine()->IsOptimizedEngine() && g_pA3DConfig->GetRunEnv() != A3DRUNENV_GFXEDITOR )
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DBillBoard::Init() Register texture in VertexCollector fail!");
			return false;
		}
	}
	return true;
...
NEXT:
	...
	if( m_nDLightID != -1 )
	{
		...
		if( pLightGrid )
		{
			pLightGrid->UpdateDLightColorOnly(m_nDLightID, ColorRGBAToColorValue(m_Color));
		}
	}
	m_nTicks ++;
	return true;
...
bool A3DBillBoard::RenderMark(A3DViewport * pCurrentViewport)
{
	...
	if( 0 )//!m_pA3DDevice->IsZBiasSupported() && g_pA3DConfig->GetRunEnv() != A3DRUNENV_GFXEDITOR )
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DBillBoard::Render add to vertex collector fail!");
			return false;
		}
	}
	else
...
bool A3DBillBoard::Render(A3DViewport * pCurrentViewport)
{
	...
	if( max(max(m_vSizeX, m_vSizeY), m_vSizeZ) < 6.0f )
	{
		A3DVECTOR3 vecPos = m_vecPos * m_matParentTM;

		// If small enough we can use BSP PVS to determine its visibility;
		A3DWorld * pA3DWorld = m_pA3DDevice->GetA3DEngine()->GetA3DWorld();
		if( pA3DWorld )
		{
			A3DScene * pA3DScene = pA3DWorld->GetA3DScene();
			if( pA3DScene )
			{
				if( !pA3DScene->IsVisiblePos(vecPos) )
					return true;
			}
		}

		// We should use some bounding box to determine its visibility against the viewport
	}
	...
	if( m_vAngle != 0.0f )
	{
		matToWorld = RotateZ(DEG2RAD(m_vAngle)) * matToWorld;
	}
	...
	for(int i=0; i<nRectCount * 4; i++)
		pVertexBuffer[i] = A3DLVERTEX(pVertPos[i], m_Color, specular, u[i % 4], v[i % 4]);
	
	if( m_pA3DDevice->GetA3DEngine()->IsOptimizedEngine() && g_pA3DConfig->GetRunEnv() != A3DRUNENV_GFXEDITOR )
	{
		if( !m_pA3DDevice->GetA3DEngine()->GetVertexCollector()->PushVertices(m_hVertexCollectorHandle,
			A3DVT_LVERTEX, pVertexBuffer, nRectCount * 4, index, nRectCount * 6) )
		{
			g_pA3DErrLog->ErrLog("A3DBillBoard::Render add to vertex collector fail!");
			return false;
		}
	...
	else
	{
		...
		m_pA3DDevice->SetLightingEnable(false);
		m_pA3DDevice->SetWorldMatrix(IdentityMatrix());
		m_pStream->SetVerts((LPBYTE) pVertexBuffer, nRectCount * 4);
		m_pStream->Appear();
		if( m_pTexture )
			m_pTexture->Appear(0);
		else
	...
	if( m_Type == A3DBILLBOARD_DYNAMICLIGHT )
	{
		...
		if( pLightGrid )
		{
			...
			if( m_nDLightID != -1 )
			{
				...
				if( pLightGrid )
				{
					pLightGrid->UpdateDLight(m_nDLightID, m_matAbsoluteTM.GetRow(3), ColorRGBAToColorValue(m_Color), m_vSizeX * 2.0f);
				}
			}
		}
	}
...
bool A3DBillBoard::Load(A3DDevice * pA3DDevice, AFile * pFileToLoad)
{
	...
	else
	{
		...
		pFileToLoad->ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
		pFileToLoad->GetStringAfter(szLineBuffer, "TextureMap: ", szResult);
...
bool A3DBillBoard::SplitMark()
{
	...
	if( m_pMarkVerts && m_pMarkIndices )
	{
		...
		if( pESP )
		{
			...
			if( !pESP->SplitMark(aabb, vecNormal, m_pMarkVerts, m_pMarkIndices, bJog, &m_nMarkVertCount, &m_nMarkIndexCount, vMarkRangeFactor) )
			{
				g_pA3DErrLog->ErrLog("A3DBillBoard::SplitMark(), Fail to split the mark on the wall");
				return false;
			}
		}

		A3DTerrain * pA3DTerrain = pCDS->GetA3DTerrain();
		if( m_nMarkVertCount == 0 && pA3DTerrain )
		{
			if( !pA3DTerrain->SplitMark(aabb, m_pMarkVerts, m_pMarkIndices, true, &m_nMarkVertCount, &m_nMarkIndexCount) )
			{
				g_pA3DErrLog->ErrLog("A3DBillBoard::SplitMark(), Fail to split the mark on the terrain");
				return false;
			}
		}

		m_pStream->SetIndices(m_pMarkIndices, m_nMarkIndexCount);	
	}
