...
A3DSuperDecal::A3DSuperDecal()
{
	...
	m_dwClassID		= A3D_CLASS_ID_SUPERDECAL;
}
...
bool A3DSuperDecal::Init(A3DDevice* pDevice, char* szTexName)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DSuperDecal::Init(), A3DDecal::Init failed!");
		return false;
	}
...
bool A3DSuperDecal::Render(A3DViewport * pView)
{
	if( m_bFinish )	return true;
	return A3DDecal::Render(pView);
}
...
bool A3DSuperDecal::Save(AFile * pFileToSave)
{
	...
	else
	{
		...
		{
			sprintf(szLineBuffer, "KeyPoint: (%d, %d, %f, %f, %x, %f, %f, %f, %f, %f",
				i, m_aKeys[i].iTickStamp, m_aKeys[i].fWidth, m_aKeys[i].fHeight, 
		...
		if( m_pTexture )
			sprintf(szLineBuffer, "TextureMap: %s", m_pTexture->GetMapFileInFolder(m_pA3DDevice->GetA3DEngine()->GetA3DGFXMan()->GetTextureFolder()));
		else
...
bool A3DSuperDecal::Load(A3DDevice * pDevice, AFile * pFileToLoad)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DSuperDecal::Load(), failed to initialize decal!");
		return false;
	}
	...
	else
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DSuperDecal::Load(), failed to change texture!");
			return false;
		}
