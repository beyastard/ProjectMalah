...
///////////////////////////////////////////////////////////////////////////
//
//	Implement of A3DVSSScene
//
///////////////////////////////////////////////////////////////////////////

A3DVSSScene::A3DVSSScene() : A3DScene()
{
	m_iNumRoute		= NULL;
	m_iCurRoute		= 0;
	m_bRenderAll	= false;
...
*/
bool A3DVSSScene::Init(A3DDevice* pA3DDevice)
{
	if (!A3DScene::Init(pA3DDevice))
	{
		g_pA3DErrLog->ErrLog("A3DVSSScene::Init, Failed to call A3DScene::Init.");
		return false;
	}
...
/*	Set current route
	...
	iIndex: route's index returned by A3DVSSScene::LoadRoute().
	iSample: current sample of specified route
*/
bool A3DVSSScene::SetCurrentRoute(int iIndex, int iSample)
{
...
*/
bool A3DVSSScene::SetCurrentSection(int iSection)
{
...
/*	Update visible surface set
	...
	Return true for success, otherwise return false.
*/
bool A3DVSSScene::UpdateVisibleSets(A3DViewport* pCurrentViewport)
{
	if (!m_nNumTextures)
		return true;
...
*/
A3DVSSScene::ROUTE* A3DVSSScene::AllocRouteSlot()
{
	int i;
	...
	if (i >= MAXNUM_ROUTE)
	{
		g_pA3DErrLog->ErrLog("A3DVSSScene::AllocRouteSlot, too many routes.");
		return NULL;
	}
...
*/
int A3DVSSScene::LoadRoute(char* szFileName, bool bAll)
{
	int iNumFace, iNumObj;

	if (!(iNumFace = GetFaceCount()))
	{
		g_pA3DErrLog->ErrLog("A3DVSSScene::LoadRoute, scene data hasn't been loaded.");
		return -1;
	}
	...
	if (!pRoute)
	{
		g_pA3DErrLog->ErrLog("A3DVSSScene::Init, failed to allocate route slot!");
		return -1;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DVSSScene::Init, failed to create route object!");
		return -1;
	}

	if (!(pRoute->pRoute->Load(szFileName, bAll)))
	{
		g_pA3DErrLog->ErrLog("A3DVSSScene::LoadRoute, failed to load route.");
		goto Error;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DVSSScene::LoadRoute, not enough memory.");
		goto Error;
	}
...
*/
int	A3DVSSScene::LoadRoute(BYTE* pBuf)
{
	int iNumFace, iNumObj;
...
/*	Remove one route
	...
	iIndex: route's index returned by A3DVSSScene::LoadRoute().
*/
bool A3DVSSScene::RemoveRoute(int iIndex)
{
	if (iIndex < 0 || iIndex >= MAXNUM_ROUTE)
		return false;
...
//	Remove all routes
void A3DVSSScene::RemoveAllRoutes()
{
...
/*	Get route loader object
	...
	iIndex: route's index returned by A3DVSSScene::LoadRoute().
*/
A3DVSSRouteLoader* A3DVSSScene::GetRouteLoader(int iIndex)
{
	if (iIndex < 0 || iIndex >= MAXNUM_ROUTE)
		return NULL;
...
*/
int	A3DVSSScene::GetSceneObjectNum()
{
	if (!m_iNumRoute)
		return 0;
...
*/
bool A3DVSSScene::GetCurrentVisObjs(int* pBuf, int* piNumVisObj)
{
