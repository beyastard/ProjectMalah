...
A3DDecal::A3DDecal()
{
	...
	m_dwClassID		= A3D_CLASS_ID_DECAL;
...
bool A3DDecal::Init(int iType, A3DDevice* pDevice, char* szTexName, bool bForceRender/* false */)
{
	...
	if (szTexName)
	{
		strncpy(m_szTextureMap, szTexName, MAX_PATH);
	...
	if (!m_bOptimized)
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DDecal::Init() not enough memory for stream!");
			return false;
		}
		
		if (!m_pA3DStream->Init(pDevice, A3DVT_TLVERTEX, 4, 6, 0))
		{
			g_pA3DErrLog->ErrLog("A3DDecal::Init(), Failed to initialize stream!");
			return false;
		}
		...
		{
			g_pA3DErrLog->ErrLog("A3DDecal::Init(), Failed to lock index buffer!");
			return false;
		}
		...
		{
			g_pA3DErrLog->ErrLog("A3DDecal::Init(), Failed to unlock index buffer!");
			return false;
		}
	}
	else
...
bool A3DDecal::Update(A3DViewport* pView)
{
	...
	if (m_iType == DECALTYPE_PURE3D)
	{
		...
		CLAMPVALUE(fWidth, m_fMinWidth, m_fMaxWidth);
		CLAMPVALUE(fHeight, m_fMinHeight, m_fMaxHeight);
	}
...
//	Render decal
bool A3DDecal::Render(A3DViewport* pView)
{
	if( !m_pA3DDevice )	return true;
