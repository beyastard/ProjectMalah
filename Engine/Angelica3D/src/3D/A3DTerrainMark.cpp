...
bool A3DTerrainMark::Init(A3DDevice * pA3DDevice, char * szTexture)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DTerrainMark::Init Not enough memory!");
		return false;
	}

	if( !m_pA3DStream->Init(m_pA3DDevice, A3DVT_LVERTEX, 4, 6, 0) )
	{
		g_pA3DErrLog->ErrLog("A3DTerrainMark::Init init stream fail!");
		return false;
	}
