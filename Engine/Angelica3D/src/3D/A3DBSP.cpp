...
bool A3DBSP::Load(char* szFileName)
{
	...
	{
		File.Close();
		g_pA3DErrLog->ErrLog("A3DBSP::Load(), Cannot open file %s!", szFileName);
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DBSP::Load(), Failed to read file header");
		File.Close();
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DBSP::Load(), Wrong file format or version");
		File.Close();
		return false;
	}
...
bool A3DBSP::ReadPlaneLump(AFile* pFile, PBSPFILEHEADER pHeader)
{
	...
	if (iNumPlane == -1)
	{
		g_pA3DErrLog->ErrLog("Failed to read plane lump A3DBSP::ReadPlaneLump");
		return false;
	}
	...
	if (!aPlanes)
	{
		g_pA3DErrLog->ErrLog("Not enough memory in A3DBSP::ReadPlaneLump");
		return false;
	}
...
bool A3DBSP::ReadNodeLump(AFile* pFile, PBSPFILEHEADER pHeader)
{
	...
	if (iNumNode == -1)
	{
		g_pA3DErrLog->ErrLog("Failed to read plane lump A3DBSP::ReadNodeLump");
		return false;
	}
	...
	if (!aNodes)
	{
		g_pA3DErrLog->ErrLog("Not enough memory in A3DBSP::ReadNodeLump");
		return false;
	}
...
bool A3DBSP::ReadLeafLump(AFile* pFile, PBSPFILEHEADER pHeader)
{
	...
	if (iNumLeaf == -1)
	{
		g_pA3DErrLog->ErrLog("Failed to read plane lump A3DBSP::ReadLeafLump");
		return false;
	}
	...
	if (!aLeaves)
	{
		g_pA3DErrLog->ErrLog("Not enough memory in A3DBSP::ReadLeafLump");
		return false;
	}
...
bool A3DBSP::ReadSurfRefLump(AFile* pFile, PBSPFILEHEADER pHeader)
{
	...
	if (iNumSurfRef == -1)
	{
		g_pA3DErrLog->ErrLog("Failed to read plane lump A3DBSP::ReadSurfRefLump");
		return false;
	}
...
bool A3DBSP::ReadPVSLump(AFile* pFile, PBSPFILEHEADER pHeader)
{
	...
	if (iPVSSize == -1)
	{
		g_pA3DErrLog->ErrLog("Failed to read plane lump A3DBSP::ReadPVSLump");
		return false;
	}
	...
	if (!m_aClusters)
	{
		g_pA3DErrLog->ErrLog("A3DBSP::ReadPVSLump(), not enough memory");
		return false;
	}
...
bool A3DBSP::InitOthers()
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DBSP::InitOthers, Not enough memory for cluster fill flags");
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DBSP::InitOthers, Not enough memory for surface rendered flags");
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DBSP::InitOthers, Not enough memory for surface index buffer");
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DBSP::InitOthers, Not enough memory for rendered cluster buffer");
		return false;
	}
...
*/
bool A3DBSP::PointInLeaf(A3DVECTOR3& vPos, int* piLeafID)
{
	A3DPLANE* pPlane;
	float d;
	int iLeaf;
...
*/
bool A3DBSP::CanBeSeenFromLeaf(A3DVECTOR3& vPos, int iLeaf)
{
	if (m_aLeaves[iLeaf].iCluster < 0)
		return false;
...
/*	Get visible draw surfaces from specified point
	...
	Return address of surface index buffer for success, otherwise return NULL

	vPos: specified position.
	aClipPlanes: clip plane of view frustum
	iNumClip: number of planes in aClipPlanes.
	piNumSurf: number of surface in returned buffer
	piLeafID: used to receive ID of leaf in which view point lies, if view point
			  is in a solid leaf or out of BSP space, -1 will be set to *piLeafID
*/
int* A3DBSP::GetVisibleSurfs(A3DVECTOR3& vPos, A3DPLANE* aClipPlanes, int iNumClip,
							 int* piNumSurf, int* piLeafID)
{
	int iLeaf;
	if (!PointInLeaf(vPos, &iLeaf))
	{
		*piLeafID	= -1;
		*piNumSurf	= 0;
		return NULL;	//	vPos is in solid leaf
	}

	bool bRet;

	m_aClipPlanes	= aClipPlanes;
	m_iNumClip		= iNumClip;
	m_vViewPos		= vPos;
...
bool A3DBSP::E_GetVisibleSurfs(int iLeaf)
{
	...
	if (iLeaf == m_iViewLeaf)
	{
		...
		for (i=0; i < m_iNumRCluster; i++)
		{
			...
			for (k=0; k < m_iNumClip; k++)
			{
				if (CLS_PlaneToAABB(m_aClipPlanes[k], pLeaf->vMins, pLeaf->vMaxs) < 0)
					goto NextRCluster;
			}
...
void A3DBSP::RecursiveWorldNode(BSPNODE* pNode, bool bNeedClip)
{
	...
	if (bNeedClip)
	{
		...
		for (i=0; i < m_iNumClip; i++)
		{
			iSide = CLS_PlaneToAABB(m_aClipPlanes[i], pNode->vMins, pNode->vMaxs);
			if (iSide < 0)
				return;
			else if (!iSide)
				bNeedClip = true;
