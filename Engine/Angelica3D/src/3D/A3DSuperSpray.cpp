...
{
	m_pA3DDevice = NULL;
	m_dwClassID = A3D_CLASS_ID_SUPERSPRAY;
	m_vOffAxis = 0.0f;
...
bool A3DSuperSpray::EmitParticles()
{
	...
	case A3DPARTICLE_STANDARD_PARTICLES:
		...
		for(n=0; n<nToNew; n++)
		{
			...
			A3DMATRIX4 matEmitDir = RotateX(DEG2RAD(vOffAxis - m_vOffAxis)) * RotateZ(DEG2RAD(vOffPlane)) * RotateX(DEG2RAD(m_vOffAxis));
	...
	case A3DPARTICLE_OBJECT_FRAGMENTS:
		g_pA3DErrLog->ErrLog("Super spray can not work with object fragments!");
		return false;
