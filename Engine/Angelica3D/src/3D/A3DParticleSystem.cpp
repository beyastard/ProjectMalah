...
{
	m_pA3DDevice	= NULL;
	m_dwClassID		= A3D_CLASS_ID_PARTICLESYSTEM;
...
bool A3DParticleSystem::CreateStandard()
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DParticleSystem::CreateStandard(), Not enough memory!");
		return false;
	}
	...
	case A3DPARTICLE_STANDARD_PARTICLE_TETRA:
		...
		break;
	default:
		g_pA3DErrLog->ErrLog("A3DParticleSystem::CreateStandard Unsupport particle type specified!");
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DParticleSystem::CreateStandard Not enough memroy!");
		return false;
	}

	if( !m_pA3DStream->Init(m_pA3DDevice, A3DVT_TLVERTEX, m_nMaxVertCount, m_nMaxIndexCount, 0, true, true) )
	{
		g_pA3DErrLog->ErrLog("A3DParticleSystem::CreateStandard m_pA3DStream Init Fail!");
		return false;
	}
...
bool A3DParticleSystem::CreateMeta()
{
	...
	g_pA3DErrLog->ErrLog("A3DParticleSystem::CreateMeta This method is not implemented yet!");
	return false;
}
...
bool A3DParticleSystem::CreateObjectFragment(int nTextureNum, A3DTexture ** ppTextures)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DParticleSystem::CreateObjectFragment Too many textures used!");
		return false;
	}
	...
	if( NULL == m_pParticleBuffer )
	{
		g_pA3DErrLog->ErrLog("A3DParticleSystem::CreateObjectFragment(), Not enough memory!");
		return false;
	}
	...
	if( !m_pA3DStream->Init(m_pA3DDevice, A3DVT_LVERTEX, m_nMaxVertCount, m_nMaxIndexCount, 0) )
	{
		g_pA3DErrLog->ErrLog("A3DParticleSystem::CreateObjectFragment Init Stream Fail!");
		return false;
	}
...
bool A3DParticleSystem::DefaultValues()
{
	...
	m_EmitterParentTM = IdentityMatrix();
...
bool A3DParticleSystem::UpdateParticles()
{
	...
	case A3DPARTICLE_META_PARTICLES:
		g_pA3DErrLog->ErrLog("Not implemented now!");
		return false;
	case A3DPARTICLE_OBJECT_FRAGMENTS:
		...
		else
		{
			...
			else
			{
				...
				if( pA3DCDS )
				{
					...
					DWORD dwMaskOld = pA3DCDS->GetModelRayTraceMask();

					pA3DCDS->ClearModelRayTraceMask(0xffffffff);
					bool bHit = pA3DCDS->RayTrace(vecStart, vecSpeed, 1.0f, &rayTrace, NULL);
					pA3DCDS->SetModelRayTraceMask(dwMaskOld);
					if( bHit )
					{
						...
						{		
							...
							if( Magnitude(vecSpeed) < 0.1f ) // Too slow touch speed, about to stop
							{
							...
							else
							{
								...
								if( pParticle->pModel->FindGroupActionByName("[落地]", NULL) )
								{
									pParticle->pModel->PlayActionByName("[落地]", true);
								}
							}
						}
...
bool A3DParticleSystem::RenderStandardParticles(A3DViewport * pCurrentViewport)
{
	...
	for(int id=0; id<m_nNumParticles; id++)
	{
		...
		case A3DPARTICLE_STANDARD_PARTICLE_TETRA:
			...
			break;
		default:
			g_pA3DErrLog->ErrLog("A3DSuperSpray::RenderParticles now you can only use A3DPARTICLE_STANDARD_PARTICLE_FACING or TETRA!");
			return false;
		}
...
bool A3DParticleSystem::RenderObjectFragmentParticles(A3DViewport * pCurrentViewport)
{
	...
	for(int id=0; id<m_nNumParticles; id++)
	{
		...
		matTM = Translate(pCommonParticle->vecPos.x, pCommonParticle->vecPos.y, pCommonParticle->vecPos.z);
		matTM = RotateAxis(pCommonParticle->vecSpinAxis, DEG2RAD(pCommonParticle->vSpinPhase)) * matTM;
...
bool A3DParticleSystem::Load(A3DDevice * pA3DDevice, AFile * pFileToLoad)
{
	...
	else
	{
		...
		for(i=0; i<m_nColorNum; i++)
		{
			...
			if( i != nval )
			{
				g_pA3DErrLog->ErrLog("A3DParticleSystem::Load() Parse file error!");
				return false;
			}
		}
...
{
	m_EmitterRelativeTM = TransformMatrix(m_EmitterDir, m_EmitterUp, m_EmitterPos);
	return UpdateAbsoluteTM();
}
...
bool A3DParticleSystem::AddInstancedModel(char * szInstancedModel)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DParticleSystem::AddInstancedModel(), Can not load model [%s]", szInstancedModel);
		return false;
	}
	...
	else
	{
		if( m_ParticleType != A3DPARTICLE_TYPE_UNKNOWN )
		{
			...
			case A3DPARTICLE_META_PARTICLES:
				...
				break;
			case A3DPARTICLE_OBJECT_FRAGMENTS:
				g_pA3DErrLog->ErrLog("A3DParticleSystem::SetParticleType(), Can not change to object fragment dynamicly!");
				break;
			case A3DPARTICLE_INSTANCED_GEOMETRY:
