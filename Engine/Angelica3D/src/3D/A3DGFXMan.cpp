...
{
	strcpy(m_szFolderName, "Gfx");
	strcpy(m_szTextureFolder, "Gfx\\Textures");
	...
	{
		g_pA3DErrLog->ErrLog("A3DGFXMan::Init() Not enough memory!");
		return false;
	}
	...
	{
		gfxListFile.Close();
		g_pA3DErrLog->ErrLog("A3DGFXMan::Init() Can not open %s", "gfxlist.txt");
		return true;
	}
	...
	while( gfxListFile.ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwRead) )
	{
		...
		if( !pFileImage->Open(m_szFolderName, szLineBuffer, AFILE_OPENEXIST) )
		{
			pFileImage->Close();
			delete pFileImage;
			g_pA3DErrLog->ErrLog("A3DGFXMan::Init() Can not open file [%s] to be a fileimage", szLineBuffer);
			return false;
		}
...
bool A3DGFXMan::LoadGFXFromFile(char * szGFXFile, A3DModel * pModel, char * szParentFrameName, bool bLinked, A3DGraphicsFX ** ppGFX,
	...
	*ppGFX = NULL;
	...
	if( !pGFX )
	{
		...
		if( g_pA3DConfig->GetRunEnv() == A3DRUNENV_PURESERVER )
		{
			...
			if( NULL == pGFX )
			{
				g_pA3DErrLog->ErrLog("A3DGFXMan::LoadGFXFromFile Not enough memory!");
				return false;
			}
		...
		{
			AFileMod_GetFullPath(szFullPath, m_szFolderName, szGFXFile);
			...
			if( !bForceFile )
			{
				...
				if( NULL == pFileImage )
				{
					g_pA3DErrLog->ErrLog("A3DGFXMan::LoadGFXFromFile() Can not find preloaded file image [%s], now try to load from file...", szFullPath);
					...
					{
						g_pA3DErrLog->ErrLog("A3DGFXMan::LoadGFXFromFile() Try to load a fileimage fail [%s]", szFullPath);
						pFileImage->Close();
						delete pFileImage;
						return false;
					}
					...
					m_PreloadedGFXFilesList.Append((LPVOID) pFileImage);
					g_pA3DErrLog->ErrLog("A3DGFXMan::LoadGFXFromFile(), Load a file image [%s] from file done!", szFullPath);
				}
			...
			else
			{
				...
				if( !pFileImage->Open(szFullPath, AFILE_OPENEXIST) )
				{
					g_pA3DErrLog->ErrLog("A3DGFXMan::LoadGFXFromFile(), Can not open file %s to be a fileimage!", szGFXFile);
					pFileImage->Close();
					delete pFileImage;
					return false;
				}
			...
			{
				g_pA3DErrLog->ErrLog("A3DGFXMan::LoadGFXFromFile Call GraphicsFX::Load Fail!");
				if( bForceFile )
				{
					pFileImage->Close();
					delete pFileImage;
	...
	if( pModel && szParentFrameName )
	{
		...
		if( NULL == pParentFrame )
		{
			pGFX->Release();
			delete pGFX;
			g_pA3DErrLog->ErrLog("A3DGFXMan::LoadGFXFromFile() Can not find parent frame [%s]", szParentFrameName);
			return false;
		}
	}
	...
	if( !pGFX->CreateAllFX(pModel, pParentFrame, bLinked, vecPos, vecDir, vecUp) )
	{
		pGFX->Release();
		delete pGFX;
		g_pA3DErrLog->ErrLog("A3DGFXMan::LoadGFXFromFile Call GraphicsFX::CreateAllFX Fail!");
		return false;
	}
...
bool A3DGFXMan::ReleaseGFX(PA3DGraphicsFX& pGFX)
{
...
bool A3DGFXMan::DeleteGFX(PA3DGraphicsFX& pGFX)
{
...
{
	m_pA3DDevice->GetA3DEngine()->BeginPerformanceRecord(A3DENGINE_PERFORMANCE_WORLD_GFXMAN_RENDER);
	ALISTELEMENT * pThisElement = m_GFXList.GetFirst();
	while( pThisElement != m_GFXList.GetTail() )
	{
		A3DGraphicsFX * pGFX = (A3DGraphicsFX *) pThisElement->pData;
	...
	m_pA3DDevice->GetA3DEngine()->EndPerformanceRecord(A3DENGINE_PERFORMANCE_WORLD_GFXMAN_RENDER);
	return true;
}
...
{
	// Release all created A3DGraphicsFX class objects;
	ALISTELEMENT * pThisElement = m_GFXList.GetFirst();
	while( pThisElement != m_GFXList.GetTail() )
	{
		A3DGraphicsFX * pGFX = (A3DGraphicsFX *) pThisElement->pData;
...
AFileImage * A3DGFXMan::FindFileImage(char * szFileName)
{
