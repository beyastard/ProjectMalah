...
DWORD MakeIDFromString(char* szStr);	//	Create a ID base on specified string
...
bool A3DImgModel::Copy(A3DImgModel* pSrc)
{
	...
	if (m_iNumGroup)
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DImgModel::Copy, Not enough memory !");
			return false;
		}
...
{
	if (m_iNumGroup != pSrc->m_iNumGroup)
	{
		g_pA3DErrLog->ErrLog("A3DImgModel::BitCopy, Dismatch objects!");
		return false;
	}
...
bool A3DImgModel::Load(A3DImageModelMan *pMan, char* szFile, int iMGIndex)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DImgModel::Load, Failed to open file %s", szFile);
		return false;
	}

	if (!Script.Open(&File))
	{
		g_pA3DErrLog->ErrLog("A3DImgModel::Load, Failed to open script file.");
		File.Close();
		return false;
	}
	...
	strcpy(m_szName, Script.m_szToken);
	m_dwID = MakeIDFromString(m_szName);

	if (!Script.MatchToken("{", false))
	{
		g_pA3DErrLog->ErrLog("A3DImgModel::Load, Failed to match '{'");
		goto Error;
	}
	...
	while (Script.PeekNextToken(true))
	{
		...
		if (!_stricmp(Script.m_szToken, "actgroupnumber"))
		{
			...
			if (m_iNumGroup)
			{
				if (!(m_aGroups = (ACTGROUP*)malloc(sizeof (ACTGROUP) * m_iNumGroup)))
				{
					g_pA3DErrLog->ErrLog("A3DImgModel::Load, Not enough memory !");
					goto Error;
				}
		...
		else	//	Is action group
		{
			...
			{
				free(m_aGroups);
				g_pA3DErrLog->ErrLog("A3DImgModel::Load, Failed to parse action group");
				goto Error;
			}
		}
	}
...
bool A3DImgModel::ParseActionGroup(AScriptFile* pFile, ACTGROUP* pGroup)
{
	...
	strcpy(pGroup->szName, pFile->m_szToken);

	//	Make ID
	pGroup->dwID = MakeIDFromString(pGroup->szName);
	...
	{
		g_pA3DErrLog->ErrLog("A3DImgModel::ParseActionGroup, Failed to match {");
		return false;
	}
	...
	while (pFile->PeekNextToken(true))
	{
		...
		//	ID of ation
		pFile->GetNextToken(true);
		pGroup->aActions[iCount] = MakeIDFromString(pFile->m_szToken);
...
*/
void A3DImgModel::UpdateShape(A3DCamera* pCamera)
{
	if (m_bNoAction)
		return;
