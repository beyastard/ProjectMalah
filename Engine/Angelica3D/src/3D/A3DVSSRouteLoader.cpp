...
bool A3DVSSRouteLoader::Load(char* szFileName, bool bAll)
{
	if (m_bReady)
	{
		g_pA3DErrLog->ErrLog("A3DVSSRouteLoader::Load, one route has been loaded.");
		return false;
	}

	if (!szFileName || !szFileName[0])
	{
		g_pA3DErrLog->ErrLog("A3DVSSRouteLoader::Load, Invalid file name.");
		return false;
	}
	...
	if (!fp)
	{
		g_pA3DErrLog->ErrLog("A3DVSSRouteLoader::Load, Failed to open file %s.", szFileName);
		return false;
	}
	...
	{
		fclose(fp);
		g_pA3DErrLog->ErrLog("A3DVSSRouteLoader::Load, File version or format is wrong.");
		return false;
	}
	...
	{
		fclose(fp);
		g_pA3DErrLog->ErrLog("A3DVSSRouteLoader::Load, Not enough memory for section information table.");
		return false;
	}
	...
	if (bAll)	//	Load all section vss data
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DVSSRouteLoader::Load, Not enough memory for compressed vss data.");
			goto Error;
		}
