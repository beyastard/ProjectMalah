...
bool A3DGraphicsFX::Init(A3DDevice * pA3DDevice)
{
	...
	m_matRelativeTM = TransformMatrix(m_vecDir, m_vecUp, m_vecPos);
	m_matParentTM = IdentityMatrix();
...
{
	//g_pA3DErrLog->ErrLog("%s, A3DGraphicsFX::Release()", GetName());
	if( m_pA3DDevice )
		m_pA3DDevice->GetA3DEngine()->DecObjectCount(A3DENGINE_OBJECT_GFX);
...
bool A3DGraphicsFX::Load(A3DDevice * pA3DDevice, AFile * pFileToLoad)
{
	...
	else
	{
		...
		pFileToLoad->GetStringAfter(szLineBuffer, "GRAPHICSFX: ", szResult);
		SetName(szResult);
		...
		for(i=0; i<nSuperSprayCount; i++)
		{
			...
			{
				g_pA3DErrLog->ErrLog("A3DGraphicsFX::Load() Not enough memory!");
				return false;
			}

			pFileToLoad->ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
			pFileToLoad->GetStringAfter(szLineBuffer, "SUPERSPRAY: ", szResult);
			pNewSuperSpray->SetName(szResult);

			pFileToLoad->ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
			if( strcmp(szLineBuffer, "{") != 0 )
			{
				g_pA3DErrLog->ErrLog("A3DGraphicsFX::Load() Graphics FX File parsed error!");
				return false;
			}
			if( !pNewSuperSpray->Load(m_pA3DDevice, pFileToLoad) )
			{
				g_pA3DErrLog->ErrLog("A3DGraphicsFX::Load() Load Super Spray Fail");
				return false;
			}
			pFileToLoad->ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
			if( strcmp(szLineBuffer, "}") != 0 )
		...
		for(i=0; i<nPArrayCount; i++)
		{
			...
			pFileToLoad->ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
			pFileToLoad->GetStringAfter(szLineBuffer, "PARRAY: ", szResult);
			...
			{
				g_pA3DErrLog->ErrLog("A3DGraphicsFX::Load() Load PArray Fail");
				return false;
			}
			pFileToLoad->ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
			if( strcmp(szLineBuffer, "}") != 0 )
		...
		for(i=0; i<nSuperDecalCount; i++)
		{
			...
			pFileToLoad->ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
			pFileToLoad->GetStringAfter(szLineBuffer, "SUPERDECAL: ", szResult);
			...
			{
				g_pA3DErrLog->ErrLog("A3DGraphicsFX::Load() Load SuperDecal Fail");
				return false;
			}
			pFileToLoad->ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
			if( strcmp(szLineBuffer, "}") != 0 )
		...
		for(i=0; i<nBillBoardCount; i++)
		{
			...
			pFileToLoad->ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
			pFileToLoad->GetStringAfter(szLineBuffer, "BILLBOARD: ", szResult);
			...
			{
				g_pA3DErrLog->ErrLog("A3DGraphicsFX::Load() Load BillBoard Fail");
				return false;
			}
			pFileToLoad->ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
			if( strcmp(szLineBuffer, "}") != 0 )
		...
		if( strstr(szLineBuffer, "PLAYEVENTCOUNT: ") )
		{
			...
			for(i=0; i<nPlayEventCount; i++)
			{
				...
				pFileToLoad->ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
				pFileToLoad->GetStringAfter(szLineBuffer, "PLAYEVENT: ", szResult);
				...
				if( !pNewEvent->Load(this, pFileToLoad) )
				{
					pNewEvent->Release();
					delete pNewEvent;
					g_pA3DErrLog->ErrLog("A3DGraphicsFX::Load(), Load PlayEvent [%s] Fail", szResult);
	...
	if( m_pA3DDevice )
		m_pA3DDevice->GetA3DEngine()->IncObjectCount(A3DENGINE_OBJECT_GFX);
	//g_pA3DErrLog->ErrLog("%s, A3DGraphicsFX::Load()", pFileToLoad->GetRelativeName());
	return true;
}
...
bool A3DGraphicsFX::Save(AFile * pFileToSave, bool bPackedUp)
{
	...
	else
	{
		...
		while( pThisElement != m_SuperSprayList.GetTail() )
		{
			...
			{
				g_pA3DErrLog->ErrLog("A3DGraphicsFX::Save Save Super Spray Fail");
				return false;
			}
			pFileToSave->WriteLine("}");
			pThisElement = pThisElement->pNext;
		...
		while( pThisElement != m_PArrayList.GetTail() )
		{
			...
			{
				g_pA3DErrLog->ErrLog("A3DGraphicsFX::Save Save PArray Fail");
				return false;
			}
			pFileToSave->WriteLine("}");
			pThisElement = pThisElement->pNext;
		...
		while( pThisElement != m_SuperDecalList.GetTail() )
		{
			...
			{
				g_pA3DErrLog->ErrLog("A3DGraphicsFX::Save Save Super Decal Fail");
				return false;
			}
			pFileToSave->WriteLine("}");
			pThisElement = pThisElement->pNext;
		...
		while( pThisElement != m_BillBoardList.GetTail() )
		{
			...
			{
				g_pA3DErrLog->ErrLog("A3DGraphicsFX::Save Save BillBoard Fail");
				return false;
			}
			pFileToSave->WriteLine("}");
			pThisElement = pThisElement->pNext;
		...
		while( pThisElement != m_PlayEventList.GetTail() )
		{
			...
			{
				g_pA3DErrLog->ErrLog("A3DGraphicsFX::Save PlayEvent Fail");
				return false;
			}
			pFileToSave->WriteLine("}");
			pThisElement = pThisElement->pNext;
								...
								A3DVECTOR3 vecPos, A3DVECTOR3 vecDir, A3DVECTOR3 vecUp)
{
	...
	m_matAbsoluteTM = m_matRelativeTM = TransformMatrix(m_vecDir, m_vecUp, m_vecPos);
	...
	if( !m_bHasCreated )
	{
		...
		while( pThisElement != m_SuperSprayList.GetTail() )
		{
			...
			case A3DPARTICLE_OBJECT_FRAGMENTS:
				g_pA3DErrLog->ErrLog("A3DGraphicsFX::CreateAllFX(), SuperSpray can not contain object fragments!");
				return false;
		...
		while( pThisElement != m_PArrayList.GetTail() )
		{
			...
			case A3DPARTICLE_META_PARTICLES:
			case A3DPARTICLE_INSTANCED_GEOMETRY:
				g_pA3DErrLog->ErrLog("A3DGraphicsFX::CreateAllFX() PArray now only support object fragment particles");
				return false;
			}
	...
	{	
		...
		{
			g_pA3DErrLog->ErrLog("A3DGraphicsFX::CreateAll, Recreated, but has parray!");
			return false;
		}
...
bool A3DGraphicsFX::FirePlayEvent(A3DGFXPlayEvent * pEvent)
{
	...
	{
	case A3DGFX_PLAYEVENT_TYPE_ITEM:
		A3DGFX_PLAYEVENT_ITEM * pItem;
		pItem = (A3DGFX_PLAYEVENT_ITEM *) pEvent->GetItemPtr();
		switch(pItem->dwClassID)
		{
		case A3D_CLASS_ID_SUPERSPRAY:
		...
		default:
			g_pA3DErrLog->ErrLog("A3DGraphicsFX::FirePlayEvent(), Unknown element class id");
			return false;
		}
		break;
	case A3DGFX_PLAYEVENT_TYPE_RANDITEM:
		g_pA3DErrLog->ErrLog("A3DGraphicsFX::FirePlayEvent(), unsupported event type used!");
		break;
	}
...
A3DGFXPlayEvent::A3DGFXPlayEvent()
{
	...
	m_Type			= A3DGFX_PLAYEVENT_TYPE_NULL;
	m_pItem			= NULL;
	m_nStartBegin	= 0;
	m_nStartEnd		= 0;
...
bool A3DGFXPlayEvent::Load(A3DGraphicsFX * pHostGFX, AFile * pFileToLoad)
{
	...
	else
	{
		...
		case A3DGFX_PLAYEVENT_TYPE_ITEM:
			...
			pFileToLoad->ReadLine(szLineBuffer, AFILE_LINEMAXLEN, &dwReadLen);
			pFileToLoad->GetStringAfter(szLineBuffer, "ITEMELEMENT: ", szResult);
			...
			if( !pItemElement )
			{
				g_pA3DErrLog->ErrLog("A3DGFXPlayEvent::Load(), Item effect [%s] missing!", szResult);
				return false;
			}

			pItem = (A3DGFX_PLAYEVENT_ITEM *) malloc(sizeof(A3DGFX_PLAYEVENT_ITEM));
			if( NULL == pItem )
			{
				g_pA3DErrLog->ErrLog("A3DGFXPlayEvent::Load(), Not enough memory!");
				return false;
			}
			...
			m_pItem = pItem;
			m_Type = A3DGFX_PLAYEVENT_TYPE_ITEM;
			break;

		case A3DGFX_PLAYEVENT_TYPE_RANDITEM:
			g_pA3DErrLog->ErrLog("A3DGFXPlayEvent::Load(), RandItem not supported now!");
			return false;

		default:
			g_pA3DErrLog->ErrLog("A3DGFXPlayEvent::Load(), Unknown event type not supported now!");
			return false;
		}
	}
...
bool A3DGFXPlayEvent::Save(AFile * pFileToSave)
{
	...
	else
	{
		...
		case A3DGFX_PLAYEVENT_TYPE_ITEM:
			A3DGFX_PLAYEVENT_ITEM * pItem;
			pItem = (A3DGFX_PLAYEVENT_ITEM *) m_pItem;
			if( pItem )
		...
		case A3DGFX_PLAYEVENT_TYPE_RANDITEM:
			g_pA3DErrLog->ErrLog("A3DGFXPlayEvent::Save(), RandItem not supported now!");
			return false;

		default:
			g_pA3DErrLog->ErrLog("A3DGFXPlayEvent::Save(), Unknown event type not supported now!");
			return false;
		}
	}
...
bool A3DGFXPlayEvent::SetActive(bool bActive)
{
	...
	if( m_bIsActive )
	{
		...
		if( m_nStartBegin == m_nStartEnd )
			...
			m_nThisStart = m_nStartBegin + A3DRandGenerator::RandUniformInt() % (m_nStartEnd - m_nStartBegin + 1);
	}
