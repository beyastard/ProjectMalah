...
bool A3DVertexCollector::Init(A3DDevice* pA3DDevice)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DVertexCollector::Init failed to init texture list.");
		return false;
	}
										  ...
										  A3DMaterial* pMaterial, A3DSHADER* pShader)
{
	...
	if (pTexture)
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DVertexCollector::RegisterTexture failed to create texture");
			return 0;
		}
	...
	if (pMaterial)
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DVertexCollector::RegisterTexture failed to create material");
			return 0;
		}
...
bool A3DVertexCollector::AllocateSlotBuffer(PTEXTURESLOT pSlot, int iVertexType)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DVertexCollector::AllocateSlotBuffer, not enough memory for stream!");
		return false;
	}

	if (!pStream->Init(m_pA3DDevice, VTCollectorToA3D(iVertexType), MAXNUM_VERTINSLOT, MAXNUM_INDEXINSLOT, 0))
	{
		g_pA3DErrLog->ErrLog("A3DVertexCollector::AllocateSlotBuffer, Failed to initialize stream!");
		return false;
	}
...
A3DVertexCollector::TEXTURESLOT* A3DVertexCollector::AllocateSlot(int iVertexType)
{
	...
	if (!pSlot)
	{
		g_pA3DErrLog->ErrLog("A3DVertexCollector::AllocateSlot not enough memory");
		return 0;
	}
	...
	if (iVertexType != -1)
	{
		...
		{
			free(pSlot);
			g_pA3DErrLog->ErrLog("A3DVertexCollector::AllocateSlot, Failed to create slot buffer!");
			return NULL;
		}
	}
									  ...
									  int iNumVert, WORD* aInIndices, int iNumIdx)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DVertexCollector::PushVertices, Failed to lock index buffer!");
		return false;
	}
	
	if (!pStream->LockVertexBuffer(0, 0, &aVerts, 0))
	{
		g_pA3DErrLog->ErrLog("A3DVertexCollector::PushVertices, Failed to lock vertex buffer!");
		return false;
	}
...
bool A3DVertexCollector::FlushBuffer(PTEXTURESLOT pSlot, PSLOTBUFFER pSlotBuf)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DVertexCollector::FlushBuffer, render failed!");
		return false;
	}
