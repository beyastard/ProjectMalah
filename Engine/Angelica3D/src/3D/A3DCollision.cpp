...
float CLS_HitPointOfOBBAndPolygon(A3DOBB& OBB, A3DVECTOR3** aVerts, int iNumVert, A3DPLANE& Plane);
...
/*	Check whether a ray collision with a 3D AABB, if true, calcualte the hit point.
	...
	vDir: ray's direction (just is vEnd-vStart, not to be normalized)
...
*/
bool CLS_RayToAABB3(A3DVECTOR3& vStart, A3DVECTOR3& vDir, A3DVECTOR3& vMins,
					A3DVECTOR3& vMaxs, A3DVECTOR3& vPoint, FLOAT* pfFraction, A3DVECTOR3& vNormal)
{
	...
	for (i=0; i < 3; i++)
	{
		if (vStart.m[i] < vMins.m[i])
		{
			...
			if (vDir.m[i] != 0.0f)
				vMaxT.m[i] = (vMins.m[i] - vStart.m[i]) / vDir.m[i];
		}
		else if (vStart.m[i] > vMaxs.m[i])
		{
			...
			//	Calcualte T distance to candidate plane
			if (vDir.m[i] != 0.0f)
				vMaxT.m[i] = (vMaxs.m[i] - vStart.m[i]) / vDir.m[i];
		}
	}
	...
	for (i=0; i < 3; i++)
	{
		if (i != iWhichPlane)
		{
			vPoint.m[i] = vStart.m[i] + vMaxT.m[iWhichPlane] * vDir.m[i];
...
/*	Check whether a ray collision with a 2D AABB, if true, calcualte the hit point.
	...
	vDir: ray's direction (just is vEnd-vStart, not to be normalized), only the first two
		  components are valid.
	vMins, vMaxs: Axis-Aligned Bounding Box
	vPoint (out): receive collision point, only the first two components are valid
*/
bool CLS_RayToAABB2(A3DVECTOR3& vStart, A3DVECTOR3& vDir, FLOAT vMins[2],
					FLOAT vMaxs[2], A3DVECTOR3& vPoint)
{
	...
	for (i=0; i < 2; i++)
	{
		if (vStart.m[i] < vMins[i])
		{
			...
			//	Calcualte T distance to candidate plane
			if (vDir.m[i] != 0.0f)
				vMaxT[i] = (vMins[i] - vStart.m[i]) / vDir.m[i];
		}
		else if (vStart.m[i] > vMaxs[i])
		{
			...
			//	Calcualte T distance to candidate plane
			if (vDir.m[i] != 0.0f)
				vMaxT[i] = (vMaxs[i] - vStart.m[i]) / vDir.m[i];
		}
	}
	...
	if (vMaxT[0] > vMaxT[1])
	{
		...
		vPoint.m[1] = vStart.m[1] + vMaxT[0] * vDir.m[1];
	...
	else	//	vMaxT[0] <= vMaxT[1]
	{
		...
		vPoint.m[0] = vStart.m[0] + vMaxT[1] * vDir.m[0];
...
/*	Do collision detection between specified ray segment and 3D OBB.
	...
	vDir: ray's direction (is just vEnd-vStart, not be normalized)
	OBB: 3D Oriented Bounding Box.
	vPoint (out): used to receive hit position if collision occured.
	pfFraction (out): hit fraction.
	vNormal (out): hit plane's normal
*/
bool CLS_RayToOBB3(A3DVECTOR3& vStart, A3DVECTOR3& vDir, A3DOBB& OBB, A3DVECTOR3& vPoint,
				   FLOAT* pfFraction, A3DVECTOR3& vNormal)
{
	...
	fDelta	= DotProduct(OBB.XAxis, vDir);
	...
	fDelta	= DotProduct(OBB.YAxis, vDir);
	...
	fDelta	= DotProduct(OBB.ZAxis, vDir);
	...
	{
		*pfFraction = fMinT;
		vPoint	= vStart + vDir * fMinT;
		vNormal	= vN1;
	}
	else
	{
		*pfFraction = fMaxT;
		vPoint	= vStart + vDir * fMaxT;
		vNormal	= vN2;
	}
...
{
	float d1, d2;
	float* vNormal	= Plane.vNormal.m;
	float* vMaxs	= _vMaxs.m;
	float* vMins	= _vMins.m;
...
__declspec( naked ) int CLS_PlaneToAABB(A3DPLANE& Plane, A3DVECTOR3& _vMins, A3DVECTOR3& _vMaxs)
{
...
/*	Check whether a triangle collision with a AABB. This function bases
	...
	v0, v1, v2: triangle's vertice.
	Plane: plane on which triangle lies
	AABB: Axis-Aligned Bounding Box. all components will be used
*/
bool CLS_TriangleToAABB(A3DVECTOR3& v0, A3DVECTOR3& v1, A3DVECTOR3& v2, 
						A3DPLANE& Plane, A3DAABB& AABB)
{
	...
	//	Then whether AABB collision triangle plane
	if (CLS_PlaneToAABB(Plane, AABB.Mins, AABB.Maxs))
		return false;
...
/*	Check whether ray intersect with triangle using the algorithm introduced by
	...
	vDir: direction of ray (needn't to be normalized)
					   ...
					   A3DVECTOR3& v1, A3DVECTOR3& v2, A3DVECTOR3& vPoint, bool b2Sides)
{
	...
	A3DVECTOR3 tvec, qvec, pvec = CrossProduct(vDir, vEdge2);
	...
	{
		if (fDet < 0.00001f && fDet > -0.00001f)
			return false;
	}
...
*/
	...
	else
	{
		...
		*pfFraction = CLS_HitPointOfOBBAndPolygon(OBB, aVerts, 4, Plane);
...
*/
int CLS_AABBMoveToPlane(A3DVECTOR3& vStart, A3DVECTOR3& vEnd, A3DVECTOR3& vExts, 
						A3DPLANE& Plane, float* pfFraction)
{
	A3DVECTOR3 vOffs;
