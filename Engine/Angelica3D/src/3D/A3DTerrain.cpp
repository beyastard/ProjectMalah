...
A3DTerrain::A3DTerrain()
{
	...
	ZeroMemory(m_pStreams, sizeof(A3DStream *) * A3DTERRAIN_MAX_TEXTURE);
	ZeroMemory(m_pTextures, sizeof(A3DTexture *) * A3DTERRAIN_MAX_TEXTURE);
	ZeroMemory(m_pVertexBuffers, sizeof(A3DLVERTEX *) * A3DTERRAIN_MAX_TEXTURE);
	ZeroMemory(m_pIndexBuffers, sizeof(WORD *) * A3DTERRAIN_MAX_TEXTURE);
	ZeroMemory(m_pExtraVertexBuffers, sizeof(A3DLVERTEX *) * A3DTERRAIN_MAX_TEXTURE);
	ZeroMemory(m_pExtraIndexBuffers, sizeof(WORD *) * A3DTERRAIN_MAX_TEXTURE);

	ZeroMemory(m_nMaxVerts, sizeof(int) * A3DTERRAIN_MAX_TEXTURE);
	ZeroMemory(m_nMaxIndices, sizeof(int) * A3DTERRAIN_MAX_TEXTURE);
	ZeroMemory(m_nMaxExtraVerts, sizeof(int) * A3DTERRAIN_MAX_TEXTURE);
	ZeroMemory(m_nMaxExtraIndices, sizeof(int) * A3DTERRAIN_MAX_TEXTURE);
...
bool A3DTerrain::Init(A3DDevice * pA3DDevice, PTERRAINPARAM pTerrainParam, char * szTexture, char * szHeightMap, char * szTextureMap)
{
	int		i, j;
	...
	m_vDetailRatio = m_vCellSize / 5.0f;
	...
	AFileMod_GetFullPath(szFullpath, m_szFolderName, m_szHeightMapName);

	file = fopen(szFullpath, "rb");
	if( NULL == file )
	{
		g_pA3DErrLog->ErrLog("A3DTerrain::Init Can not open height map: %s", szFullpath);
		return false;
	}
	...
	{
		g_pA3DErrLog->ErrLog("A3DTerrain::Init Not Enough Memory!");
		return false;
	}
	...
	// read the height map;
	if( strstr(m_szHeightMapName, ".h16") )
	{
	...
	else if( strstr(m_szHeightMapName, ".bin") )
	{
		for(int y=0; y<m_nHeight + 1; y++)
		{
			for(int x=0; x<m_nWidth + 1; x++)
	...
	// Load texture index;
	AFileMod_GetFullPath(szFullpath, m_szFolderName, m_szTextureMapName);
	file = fopen(szFullpath, "rb");
	if( NULL == file )
	{
		g_pA3DErrLog->ErrLog("A3DTerrain::Init Can not open texture map: %s", szFullpath);
		return false;
	}	
	...
	for(i=0; i<m_nHeight / m_nTextureCover; i++)
	{
		for(j=0; j<m_nWidth / m_nTextureCover; j++)
		{
			...
			{
				g_pA3DErrLog->ErrLog("A3DTerrain::Init The texture map contained more than m_nNumTexture index, pos[%d, %d]", j, i);
				return false;
			}
		}
	}
	...
	else
	{
		...
		case 16:
			...
			break;
		default:
			g_pA3DErrLog->ErrLog("Too large texture cover [%d] or it is not a power of 2! The max is 16!", m_nTextureCover);
			return false;
		}

		if( (m_nWidth % m_nTextureCover) || (m_nHeight % m_nTextureCover) )
		{
			g_pA3DErrLog->ErrLog("m_nWidth[%d] and m_nHeight[%d] must be dividable by m_nTextureCover[%d]!", m_nWidth, m_nHeight, m_nTextureCover);
			return false;
		}
		...
		{
			g_pA3DErrLog->ErrLog("A3DTerrain::Init m_nSightRange must be dividable by m_nTextureCover");
			return false;
		}
		...
		for(i=0; i<m_nNumTexture; i++)
		{
			...
			{
				g_pA3DErrLog->ErrLog("A3DTerrain::Init Stream Init Fail!");
				return false;
			}*/
		...
		if( m_pA3DDevice->IsDetailMethodSupported() )
		{
			...
			{
				g_pA3DErrLog->ErrLog("A3DTerrain::Init Init Detail Stream Fail!");
				return false;
			}*/
		}
	}
...
bool A3DTerrain::Release()
{
	...
	for(i=0; i<A3DTERRAIN_MAX_TEXTURE; i++)
	{
	...
	*/
	...
	for(int i=0; i<nDiffCount; i++)
	{
			...
			m_nExtraVisibleFaces[newTex] * 3 + 6 * 8 * 8 > m_nMaxExtraIndices[newTex] )
		{
			...
			{
				g_pA3DErrLog->ErrLog("A3DTerrain::SmoothConnection(), Not Enough Memory!");
				return false;
			}
...
bool A3DTerrain::UpdateStream()
{
			...
			{   
				...
				if( bInSight )
				{
						...
						m_nVisibleFaces[nTexIndex] * 3 + 6 * 8 * 8 > m_nMaxIndices[nTexIndex] )
					{
						...
						{
							g_pA3DErrLog->ErrLog("A3DTerrain::UpdateStream(), Not Enough Memory!");
							return false;
						}
	...
	if( m_pA3DDevice->IsDetailMethodSupported() && (m_ptCamPos.x != m_ptCamPosOld.x || m_ptCamPos.y != m_ptCamPosOld.y) )
	{
		...
		FLOAT vDetailRatio = m_vDetailRatio;
...
bool A3DTerrain::RenderExtraByTexture(int idTexture)
{
	if( m_nExtraVisibleVerts[idTexture] > 0 )
	{
		...
		if( m_bShowTerrain )
		{
			...
			{
				g_pA3DErrLog->ErrLog("A3DTerrain::Render DrawIndexedPrimitive Normal Fail!");
				return false;
			}
		}
...
bool A3DTerrain::Render(A3DViewport * pViewport)
{
	...
	m_pA3DDevice->GetD3DDevice()->SetTextureStageState(0, D3DTSS_ALPHAOP, D3DTOP_MODULATE);
...
bool A3DTerrain::SetCamera(A3DCamera * pCamera)
{
...
*/
bool A3DTerrain::RayTrace(A3DVECTOR3& vecStart, A3DVECTOR3& vecVelocity, FLOAT vTime, RAYTRACE * pRayTrace)
{
	...
	vVelocity = Magnitude(vecVelocity);
...
/*
	...
	RETURN:
		...
		false				not intersect;
*/
bool A3DTerrain::RayPickInCell(int x, int y, A3DVECTOR3& vecStart, A3DVECTOR3& vecDir, FLOAT vDis, RAYTRACE * pRayTrace)
{
	FLOAT t, u, v;
	A3DVECTOR3 v0, v1, v2;
	int nCheckCount = 0;
...
#define ERR_T	1e-6f
	...
	while( 1 ) 
	{
		...
		pRayTrace->objectType	= TRACE_OBJECT_TERRAIN;
		return true;
	}
...
bool A3DTerrain::OBBTrace(OBBTRACE * pOBBTrace, A3DOBB& obb, OBBSHAPE& obbShape)
{
	//Here we only check if the dest center is below the terrain height;
				...
				if( s == 0 )
				{
					...
					for(m=0; m<=m_nStageCover[s]; m++)
					{
						for(n=0; n<=m_nStageCover[s]; n++)
						{
							...
							vSquareError += Magnitude(GetFaceNormal(nX, nY, 0) - vecAvgNormal) * Magnitude(GetFaceNormal(nX, nY, 1) - vecAvgNormal) * 400.0f;
						}
					}
...
bool A3DTerrain::CalculateDistanceTable()
{
	...
	for(int y=0; y<m_nHeight; y++)
	{
		for(int x=0; x<m_nWidth; x++)
		{
			...
			vecDelta.y = 0.0f;
			m_pCellDistanceTable[y * m_nWidth + x] = Magnitude(vecDelta) / m_vCellSize;
		}
	}
...
bool A3DTerrain::ShowStageCellLevelInfo()
{
	...
	TextOut(hdc, 300, 0, szInfo, strlen(szInfo));
	::ReleaseDC(NULL, hdc);
	return true;
}
...
bool A3DTerrain::Create(A3DDevice * pA3DDevice, int nNumTexture, int nWidth, int nHeight, int nSightRange, float vCellSize)
{
	int		i;
	...
	if( NULL == m_pHeightBuffer )
	{
		g_pA3DErrLog->ErrLog("A3DTerrain::Create Not Enough Memory!");
		return false;
	}
	...
	else
	{
		...
		case 16:
			...
			break;
		default:
			g_pA3DErrLog->ErrLog("Too large texture cover [%d]! The max is 16!", m_nTextureCover);
			return false;
		}
		...
		if( m_nSightRange % m_nTextureCover )
		{
			g_pA3DErrLog->ErrLog("A3DTerrain::Create m_nSightRange must be dividable by m_nTextureCover");
			return false;
		}
		...
		for(i=0; i<m_nNumTexture; i++)
		{
			...
			if( !m_pStreams[i]->Init(m_pA3DDevice, A3DVT_LVERTEX, m_nMaxVertexCount, m_nMaxIndexCount, 0, true, true) )
			{
				g_pA3DErrLog->ErrLog("A3DTerrain::Create Stream Init Fail!");
				return false;
			}*/
		...
		//Init Detail Texture Sepcification Now!
		m_nDetailWidth = m_nDetailHeight = int(90.0f / m_vCellSize * 4.0f);
		if( m_nDetailWidth < 4 )
			m_nDetailWidth = m_nDetailHeight = 4;
		if( m_nDetailWidth > 20 )
			m_nDetailWidth = m_nDetailHeight = 20;
		m_vDetailRatio = m_vCellSize / 5.0f;
		...
		if( m_pA3DDevice->IsDetailMethodSupported() )
		{
			...
			if( !m_pDetailStream->Init(m_pA3DDevice, A3DVT_LVERTEX, m_nDetailVertCount, m_nDetailFaceCount * 3, 0, true, true) )
			{
				g_pA3DErrLog->ErrLog("A3DTerrain::Create Init Detail Stream Fail!");
				return false;
			}*/
		}
	}
...
bool A3DTerrain::IncreaseTextureNum(int nTextureIncrease)
{
	...
	if( nNewNumTexture > A3DTERRAIN_MAX_TEXTURE )
	{
		return false;
	}
	...
	for(int i=m_nNumTexture; i<nNewNumTexture; i++)
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DTerrain::Init Textures Init Fail!");
			return false;
		}
		/*
		m_pStreams[i] = new A3DStream();
...
bool A3DTerrain::GetTerrainColor(const A3DVECTOR3& vecPos, A3DCOLOR * pDiffuse, A3DCOLOR * pSpecular, BYTE byteAlpha)
{
	...
	else
	{
		...
		if( ((x + y) % 2) == 0 )
		{
			...
			{
				//Use Right Triangle;
				DecompressColor(color[1], &r0, &g0, &b0);
				DecompressColor(color[0], &r1, &g1, &b1);
				DecompressColor(color[3], &r2, &g2, &b2);
				vX = 1.0f - vX;
			}
			else
			{
				//Use Left Triangle;
				DecompressColor(color[2], &r0, &g0, &b0);
				DecompressColor(color[3], &r1, &g1, &b1);
				DecompressColor(color[0], &r2, &g2, &b2);
				vY = 1.0f - vY;
			}
		}
		else
		{
			...
			{
				//Use Below Triangle;
				DecompressColor(color[3], &r0, &g0, &b0);
				DecompressColor(color[2], &r1, &g1, &b1);
				DecompressColor(color[1], &r2, &g2, &b2);
				vX = 1.0f - vX;
				vY = 1.0f - vY;
			}
			else
			{
				//Use Up Triangle;
				DecompressColor(color[0], &r0, &g0, &b0);
				DecompressColor(color[1], &r1, &g1, &b1);
				DecompressColor(color[2], &r2, &g2, &b2);
			}
		}
...
bool A3DTerrain::SplitMark(A3DAABB& aabb, A3DLVERTEX* aVerts, WORD* aIndices, bool bJog, 
...
//clip in one cell;
bool A3DTerrain::ClipInCell(A3DAABB& aabb, int nCellX, int nCellY, TERRAIN_MARKSPLIT * pMarkSplit)
{
	A3DVECTOR3			aVerts[4];
	WORD				aIndices[6];
	A3DVECTOR3			vNormal;
	...
	for(int idTri=0; idTri<2; idTri++)
	{
		...
		if (pMarkSplit->nNumVert + iNumNew > A3DTERRAIN_MAXMARKVERT ||
			pMarkSplit->nNumIndices + (iNumNew - 2) * 3 > A3DTERRAIN_MAXMARKINDEX)
			return false;
