...
bool A3DPlants::Init(A3DDevice * pA3DDevice)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DPlants::Init, Not enough memory!");
		return false;
	}

	if( !m_pA3DStream->Init(m_pA3DDevice, A3DVT_LVERTEX, VERTEX_MAX_NUM, FACE_MAX_NUM * 3, 0, true, true) )
	{
		g_pA3DErrLog->ErrLog("A3DPlants::Init, Init A3DStream fail!");
		return false;
	}*/
...
bool A3DPlants::AddTreeType(char * szTreeFile, char * szSpriteFile, int nDegNum, int nTexRow, int nTexCol, FLOAT vTexDU, FLOAT vTexDV, DWORD * pTreeHandle, A3DVECTOR3& vecScale)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DPlants::AddTreeType(), Too many tree types have been added!");
		return false;
	}
	
	if( !m_pA3DDevice->GetA3DEngine()->GetA3DMoxMan()->LoadMoxFile(szTreeFile, &pTreeFrame) )
	{
		g_pA3DErrLog->ErrLog("A3DPlants::AddTreeType(), Load tree file [%s] fail!", szTreeFile);
		return false;
	}
	...
	if( m_bHWIPlants )
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DPlants::AddTreeType(), the tree file [%s] contains hyrarchies, this is not supported!", szTreeFile);
			return false;
		}
		...
		{
			g_pA3DErrLog->ErrLog("A3DPlants::AddTreeType(), the tree [%s] must have one (and only one) BB_ dummy object on it!", szTreeFile);
			return false;
		}
	}
	else
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DPlants::AddTreeType(), the tree file [%s] is empty!", szTreeFile);
			return false;
		}
		...
		while( pTreeMesh )
		{
			...
			{
				g_pA3DErrLog->ErrLog("A3DPlants::AddTreeType(), the tree file [%s] used more than %d textures!", szTreeFile, MAX_TEXTURE);
				return false;
			}
		...
		AFileMod_GetFilePath(szTreeFile, szRelativePath, MAX_PATH);
		...
		treeType.vWidth = max(pTreeFrame->GetFrameAutoAABB(0).Extents.x, pTreeFrame->GetFrameAutoAABB(0).Extents.z);
	}
...
{
	A3DMATRIX4 matScale = Scaling(pTreeType->vecScale.x, pTreeType->vecScale.y, pTreeType->vecScale.z);
	...
	for(int i=0; i<pTreeType->nNumTree; i++)
	{
		...
		m_pTrees[idTree].matTM = matScale * TransformMatrix(vecDir, vecUp, vecPos);
		...
		else
		{
			...
			ClearAABB(treeAABB.Mins, treeAABB.Maxs);
			ExpandAABB(treeAABB.Mins, treeAABB.Maxs, treeOBB);
			CompleteAABB(&treeAABB);
...
bool A3DPlants::AddTree(char * szTreeFile, TREEINFO * pTreeInfo)
{
	...
	if( NULL == pTreeType )
	{
		g_pA3DErrLog->ErrLog("A3DPlants::AddTree(), Can not find tree type [%s]!", szTreeFile);
		return false;
	}
...
void A3DPlants::UpdateTreeInfo(int nTreeIndex, TREEINFO * pTreeInfo)
{
	...
	m_pTrees[nTreeIndex].matTM = TransformMatrix(pTreeInfo->vecDir, pTreeInfo->vecUp, pTreeInfo->vecPos);
...
bool A3DPlants::AddTree(DWORD dwTreeHandle, TREEINFO * pTreeInfo, int * pTreeIndex)
{
	...
	m_pTrees[m_nNumTree].matTM = matScale * TransformMatrix(pTreeInfo->vecDir, pTreeInfo->vecUp, pTreeInfo->vecPos);
	...
	else
	{
		...
		ClearAABB(treeAABB.Mins, treeAABB.Maxs);
		ExpandAABB(treeAABB.Mins, treeAABB.Maxs, obb);
		CompleteAABB(&treeAABB);
...
bool A3DPlants::DelTree(int nTreeIndex, int * pNewTreeIndex)
{
	if( nTreeIndex >= m_nNumTree )
	{
		g_pA3DErrLog->ErrLog("A3DPlants::DelTree(), the index [%d] is beyond the tree num [%d]", nTreeIndex, m_nNumTree);
		return false;
	}
...
bool A3DPlants::AddGrassType(char * szGrassFile, DWORD * pGrassHandle, A3DVECTOR3& vecScale)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DPlants::AddGrassType(), Too many grass types have been added!");
		return false;
	}
	
	if( !m_pA3DDevice->GetA3DEngine()->GetA3DMoxMan()->LoadMoxFile(szGrassFile, &pGrassFrame) )
	{
		g_pA3DErrLog->ErrLog("A3DPlants::AddGrassType(), Load grass file [%s] fail!", szGrassFile);
		return false;
	}

	if( pGrassFrame->GetNumChilds() > 0 )
	{
		g_pA3DErrLog->ErrLog("A3DPlants::AddGrassType(), the grass file [%s] contains hyrarchies, this is not supported!", szGrassFile);
		return false;
	}
	if( pGrassFrame->GetNumMeshes() == 0 )
	{
		g_pA3DErrLog->ErrLog("A3DPlants::AddGrassType(), the grass file [%s] is empty!", szGrassFile);
		return false;
	}
	...
	while( pGrassMesh )
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DPlants::AddGrassType(), the grass file [%s] used more than %d textures!", szGrassFile, MAX_TEXTURE);
			return false;
		}
...
bool A3DPlants::UpdateAllGrassesScale(A3DPlants::GRASSTYPE * pGrassType)
{
	...
	A3DMATRIX4 matScale = Scaling(pGrassType->vecScale.x, pGrassType->vecScale.y, pGrassType->vecScale.z);
	...
	for(int i=0; i<pGrassType->nNumGrass; i++)
	{
		...
		m_pGrasses[idGrass].matTM = matScale * TransformMatrix(vecDir, vecUp, vecPos);
	}
	return true;
}
...
void A3DPlants::UpdateGrassInfo(int nGrassIndex, GRASSINFO * pGrassInfo)
{
	...
	m_pGrasses[nGrassIndex].matTM = TransformMatrix(pGrassInfo->vecDir, pGrassInfo->vecUp, pGrassInfo->vecPos);
...
bool A3DPlants::AddGrass(DWORD dwGrassHandle, GRASSINFO * pGrassInfo, int * pGrassIndex)
{
	...
	GRASSTYPE * pGrassType = (GRASSTYPE *) dwGrassHandle;
	A3DMATRIX4  matScale = Scaling(pGrassType->vecScale.x, pGrassType->vecScale.y, pGrassType->vecScale.z) *
		Scaling(pGrassInfo->vecScale.x, pGrassInfo->vecScale.y, pGrassInfo->vecScale.z);

	m_pGrasses[m_nNumGrass].pType = pGrassType;
	m_pGrasses[m_nNumGrass].matTM = matScale * TransformMatrix(pGrassInfo->vecDir, pGrassInfo->vecUp, pGrassInfo->vecPos);
	m_pGrasses[m_nNumGrass].nFrame = rand() % pGrassType->pMeshes[0]->GetFrameCount();
...
bool A3DPlants::DelGrass(int nGrassIndex, int * pNewGrassIndex)
{
	...
	if( nGrassIndex >= m_nNumGrass )
	{
		g_pA3DErrLog->ErrLog("A3DPlants::DelGrass(), the index [%d] is beyond the grass num [%d]", nGrassIndex, m_nNumGrass);
		return false;
	}
...
bool A3DPlants::RenderTrees(A3DViewport * pCurrentViewport)
{
	...
	for(i=0; i<TREE_MAX_TYPE; i++)
	{
		for(j=0; j<m_pTreeTypes[i].nNumTree; j++)
		{
			...
			vDisToCam = Magnitude(vecFromCam) * pCamera->GetFOV() / DEG2RAD(65.0f);
			...
			for(k=0; k<m_pTreeTypes[i].nNumTex; k++)
			{
				...
				{
					g_pA3DErrLog->ErrLog("A3DPlants::Render(), RenderToBuffer Fail!");
					return false;
				}
	...
	for(i=0; i<GRASS_MAX_TYPE; i++)
	{
		for(j=0; j<m_pGrassTypes[i].nNumGrass; j++)
		{
			...
			for(k=0; k<m_pGrassTypes[i].nNumTex; k++)
			{
				...
				{
					g_pA3DErrLog->ErrLog("A3DPlants::RenderGrasses(), RenderToBuffer Fail!");
					return false;
				}
...
bool A3DPlants::AABBTrace(PAABBTRACEINFO pInfo, PAABBTRACE pTrace)
{
	...
	if (bHit)	//	Yes collided
	{
		...
		pTrace->objectType	= TRACE_OBJECT_PLANTS;
		pTrace->pModel		= NULL;
		pTrace->pModelOBB	= NULL;
		return true;
	}
...
bool A3DPlants::RayTrace(A3DVECTOR3& vecStart, A3DVECTOR3& vecDelta, RAYTRACE * pRayTrace)
{
		...
		if( CLS_RayToAABB3(vecStart, vecDelta, m_pTrees[i].aabb.Mins, m_pTrees[i].aabb.Maxs, vecPoint, &vFraction, vecNormal) )
		{
			if( CLS_RayToOBB3(vecStart, vecDelta, m_pTrees[i].obb, vecPoint, &vFraction, vecNormal) )
			{
				if( vFraction < pRayTrace->fFraction )
				{
					...
					pRayTrace->objectType	= TRACE_OBJECT_PLANTS;
					pRayTrace->pModel		= NULL;
					pRayTrace->pModelOBB	= NULL;
					pRayTrace->dwExtraData  = (DWORD) i;
				}
			}
		}
	}
