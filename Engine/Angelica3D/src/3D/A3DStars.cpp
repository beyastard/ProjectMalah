					...
					FLOAT vPointSize, char * pszStarTexture)
{
	...
	if( pszStarTexture )
	{
		strncpy(m_szStarTexture, pszStarTexture, MAX_PATH);
		if( !m_pA3DDevice->GetA3DEngine()->GetA3DTextureMan()->LoadTextureFromFileInFolder(m_szStarTexture, "Textures\\Sky", &m_pStarTexture, A3DTF_MIPLEVEL, 1) )
		{
			g_pA3DErrLog->ErrLog("A3DStars::Init(), Can not load texture [%s]", m_szStarTexture);
			return false;
		}
	}
...
bool A3DStars::Render(A3DViewport * pCurrentViewport)
{
	...
	if( pCamera )
	{
		...
		matWorldMatrix = Translate(vecPos.x, vecPos.y, vecPos.z);
...
bool A3DStars::TickAnimation()
{
	...
	{
		DWORD	dwBlinkTick = m_pA3DDevice->GetA3DEngine()->GetEngineTicks() + i;
		DWORD	dwBlinkInterval	= m_dwBlinkInterval;

		if( m_dwBlinkDistance > 0 )
			dwBlinkInterval += i % m_dwBlinkDistance;

		FLOAT	vAmplitudeFactor = (g_pA3DMathUtility->SIN((dwBlinkTick % dwBlinkInterval) * 360.0f / dwBlinkInterval) + 1.0f) / 2.0f; 
...
bool A3DStars::GenerateStars()
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DStars::GenerateStars(), Not enough memory!");
		return false;
	}

	DWORD	dwOldSeed = A3DRandGenerator::GetSeed();
	A3DRandGenerator::SetSeed(m_dwRandSeed);

	FLOAT		vDeg, vPitch;
	A3DVECTOR3	vecPos;
	for(int i=0; i<m_nNumStars; i++)
	{
		vDeg = A3DRandGenerator::RandUniformFloat(0.0f, 720.0f);
		vPitch = A3DRandGenerator::RandUniformFloat(-360.0f, 360.0f);
		...
		vecPos.x = A3DRandGenerator::RandUniformFloat(-1.0f, 1.0f);
		vecPos.y = A3DRandGenerator::RandUniformFloat(-1.0f, 1.0f);
		vecPos.z = A3DRandGenerator::RandUniformFloat(-1.0f, 1.0f);
	...
	A3DRandGenerator::SetSeed(dwOldSeed);
	return true;
}
...
bool A3DStars::SetStarTexture(char * szStarTexture)
{
	if( m_pStarTexture )
	{
		m_pA3DDevice->GetA3DEngine()->GetA3DTextureMan()->ReleaseTexture(m_pStarTexture);
	...
	strncpy(m_szStarTexture, szStarTexture, MAX_PATH);
	if( !m_pA3DDevice->GetA3DEngine()->GetA3DTextureMan()->LoadTextureFromFileInFolder(m_szStarTexture, "Textures\\Sky", &m_pStarTexture) )
	{
		g_pA3DErrLog->ErrLog("A3DStars::SetStarTexture(), Can not load texture [%s]", m_szStarTexture);
		return false;
	}
	return true;
}