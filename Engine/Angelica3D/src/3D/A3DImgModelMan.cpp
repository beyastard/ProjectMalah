...
*/
DWORD MakeIDFromString(char* szStr)
{
	DWORD dwID = 0;

    for (int i=0; szStr[i]; i++)
       dwID = (dwID + (i+1) * szStr[i]) % 0x8000000bu * 0xffffffefu;

	return dwID ^ 0x12345678;
}

//	Action sort compare function
int _IMActionSortCompareFunc(const void *arg1, const void *arg2)
{
...
int _IMMarkPtSortCompareFunc(const void *arg1, const void *arg2)
{
...
A3DImageModelMan::A3DImageModelMan()
{
	...
	memset(m_szFolderName, 9, sizeof (m_szFolderName));
...
int A3DImageModelMan::LoadModelGroup(char* szDescFile)
{
	...
	APath_GetFullPath(szPath, "", szDescFile);

	if (!File.Open(szPath, AFILE_OPENEXIST | AFILE_BINARY))
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::LoadModelGroup, Failed to open file %s", szDescFile);
		return -1;
	}

	if (!Script.Open(&File))
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::LoadModelGroup, Failed to open script file.");
		File.Close();
		return -1;
	}
	...
	if (!pGroup)
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::LoadModelGroup, Failed to allocate new group");
		Script.Close();
		return -1;
	}

	pGroup->dwID = MakeIDFromString(szDescFile);

	//	First, load textures section
	if (!Script.MatchToken("sec:textures", false))
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::LoadModelGroup, Failed to find 'sec:textures' section.");
		goto Error;
	}
	else
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DImageModelMan::LoadModelGroup, Failed to parse textures section");
			goto Error;
		}
	}
...
//	Script.ResetScriptFile();
	...
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::LoadModelGroup, Failed to find 'sec:actions' section.");
		goto Error;
	}
	else if (!ParseActionSectionInGroupFile(pGroup, &Script))
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::LoadModelGroup, Failed to parse actions section");
		goto Error;
	}
...
//	Script.ResetScriptFile();
	...
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::LoadModelGroup, Failed to find 'sec:cells' section.");
		goto Error;
	}
	else if (!ParseCellSectionInGroupFile(pGroup, &Script))
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::LoadModelGroup, Failed to parse cells section");
		goto Error;
	}
...
//	Script.ResetScriptFile();
	...
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::LoadModelGroup, Failed to find 'sec:models' section.");
		goto Error;
	}
	else
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DImageModelMan::LoadModelGroup, Failed to parse image models section");
			goto Error;
		}
	}
...
//	Script.ResetScriptFile();
	...
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::LoadModelGroup, Failed to find 'sec:textureinfo' section.");
		goto Error;
	}
	else if (!ParseTexInfoSectionInGroupFile(pGroup, &Script))
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::LoadModelGroup, Failed to parse textureinfo section");
		goto Error;
	}
...
//	Script.ResetScriptFile();
	...
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::LoadModelGroup, Failed to find 'sec:markpoints' section.");
		goto Error;
	}
	else if (!ParseMarkPtSectionInGroupFile(pGroup, &Script))
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::LoadModelGroup, Failed to parse mark points section");
		goto Error;
	}
...
A3DImageModelMan::MODELGROUP* A3DImageModelMan::AllocEmptyGroup()
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::AllocEmptyGroup, Too many model groups");
		return NULL;
	}
													  ...
													  char* szTexPath)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::ParseTextureSectionInGroupFile, Failed to match {");
		return false;
	}
	...
	while (pFile->PeekNextToken(true))
	{
		...
		if (!_stricmp(pFile->m_szToken, "texturenumber"))
		{
			...
			if (pGroup->iNumTexture)
			{
				...
				{
					g_pA3DErrLog->ErrLog("A3DImageModelMan::ParseTextureSectionInGroupFile, not enough memory!");
					return false;
				}
		...
		else
		{
			...
			if (!pTexMan->LoadTextureFromFile(szFile, &pTexture, A3DTF_MIPLEVEL, 1))
				g_pA3DErrLog->ErrLog("A3DImageModelMan::ParseTextureSectionInGroupFile, Failed to load texture %s", szFile);
			else
...
bool A3DImageModelMan::ParseActionSectionInGroupFile(MODELGROUP* pGroup, AScriptFile* pFile)
{
	if (!pFile->MatchToken("{", false))
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::ParseActionSectionInGroupFile, Failed to match {");
		return false;
	}
	...
	while (pFile->PeekNextToken(true))
	{
		...
		if (!_stricmp(pFile->m_szToken, "actionnumber"))
		{
			...
			if (pGroup->iNumAction)
			{
				...
				{
					g_pA3DErrLog->ErrLog("A3DImageModelMan::ParseActionSectionInGroupFile, not enough memory!");
					return false;
				}
		...
		else
		{
			...
			//	ID
			pAct->dwID = MakeIDFromString(pAct->szName);
...
bool A3DImageModelMan::ParseCellSectionInGroupFile(MODELGROUP* pGroup, AScriptFile* pFile)
{
	if (!pFile->MatchToken("{", false))
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::ParseCellSectionInGroupFile, Failed to match {");
		return false;
	}
	...
	while (pFile->PeekNextToken(true))
	{
		...
		if (!_stricmp(pFile->m_szToken, "cellnumber"))
		{
			...
			if (pGroup->iNumCell)
			{
				...
				{
					g_pA3DErrLog->ErrLog("A3DImageModelMan::ParseCellSectionInGroupFile, not enough memory!");
					return false;
				}
													...
													char* szModelPath)
{
	if (!pFile->MatchToken("{", false))
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::ParseModelSectionInGroupFile, Failed to match {");
		return false;
	}
	...
	while (pFile->PeekNextToken(true))
	{
		...
		if (!_stricmp(pFile->m_szToken, "modelnumber"))
		{
			...
			if (pGroup->iNumModel)
			{
				...
				{
					g_pA3DErrLog->ErrLog("A3DImageModelMan::ParseModelSectionInGroupFile, not enough memory!");
					return false;
				}
		...
		else
		{
			...
			if (!(pModel = LoadImageModel(szFile, pGroup-m_aGroups)))
				g_pA3DErrLog->ErrLog("A3DImageModelMan::ParseModelSectionInGroupFile, Failed to load image model file %s", szFile);
			else
...
bool A3DImageModelMan::ParseTexInfoSectionInGroupFile(MODELGROUP* pGroup, AScriptFile* pFile)
{
	if (!pFile->MatchToken("{", false))
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::ParseTexInfoSectionInGroupFile, Failed to match {");
		return false;
	}
...
bool A3DImageModelMan::ParseMarkPtSectionInGroupFile(MODELGROUP* pGroup, AScriptFile* pFile)
{
	if (!pFile->MatchToken("{", false))
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::ParseMarkPtSectionInGroupFile, Failed to match {");
		return false;
	}
	...
	while (pFile->PeekNextToken(true))
	{
		...
		if (!_stricmp(pFile->m_szToken, "marknumber"))
		{
			...
			if (pGroup->iNumMarkPt)
			{
				...
				{
					g_pA3DErrLog->ErrLog("A3DImageModelMan::ParseMarkPtSectionInGroupFile, not enough memory!");
					return false;
				}
		...
		else
		{
			...
			pMarkPt->dwID = MakeIDFromString(pMarkPt->szName);
		}
...
A3DImageModelMan::TEXTURE* A3DImageModelMan::AllocTextureSlot(MODELGROUP* pGroup, int iCount)
{
	...
	if (!pSlot->aRModels)
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::AllocTextureSlot, not enough memory!");
		return NULL;
	}
...
A3DImgModel* A3DImageModelMan::LoadImageModel(char* szModelFile, int iGroup)
{
	...
	if (!pModel)
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::LoadImageModel, not enough memory!");
		return NULL;
	}

	if (!pModel->Load(this, szModelFile, iGroup))
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::LoadImageModel, Failed to initiaize image model!");
		return NULL;
	}
...
A3DImgModel* A3DImageModelMan::FindImageModel(int iGroup, char* szName)
{
	...
	MODELGROUP* pGroup = &m_aGroups[iGroup];
	DWORD dwID = MakeIDFromString(szName);
...
A3DImgModel* A3DImageModelMan::CreateImageModel(int iGroup, char* szName)
{
	...
	if (!pDest)
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::CreateImageModel, not enough memory!");
		return NULL;
	}
	...
	if (!CopyImageModel(pDest, iGroup, szName))
	{
		pDest->Release();
		delete pDest;
		g_pA3DErrLog->ErrLog("A3DImageModelMan::CreateImageModel, Failed to copy image model!");
		return NULL;
	}
...
bool A3DImageModelMan::CopyImageModel(A3DImgModel* pDest, int iGroup, char* szName)
{
	...
	if (!pSrc)
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::CopyImageModel, Failed to find iamge mode %s!", szName);
		return NULL;
	}
...
bool A3DImageModelMan::BitCopyImageModel(A3DImgModel* pDest, int iGroup, char* szName)
{
	...
	if (!pSrc)
	{
		g_pA3DErrLog->ErrLog("A3DImageModelMan::BitCopyImageModel, Failed to find iamge mode %s!", szName);
		return NULL;
	}
...
bool A3DImageModelMan::AddRenderModelToQueue(TEXTURE* pSlot, A3DImgModel* pModel)
{
	...
	if (pSlot->iNumRModel >= pSlot->iMaxRModel)
	{
		...
		A3DImgModel** pNewBuf = (A3DImgModel**)realloc(pSlot->aRModels, sizeof (A3DImgModel*) * iNewSize);
		if (!pNewBuf)
		{
			//	Not enough memory to extend buffer, so discard pModel
			g_pA3DErrLog->ErrLog("A3DImageModelMan::AddRenderModelToQueue, Not enough memory to expand buffer!");
			return false;
		}

		pSlot->aRModels		= pNewBuf;
