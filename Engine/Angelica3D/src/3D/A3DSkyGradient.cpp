...
bool A3DSkyGradient::Init(A3DDevice * pA3DDevice, A3DCamera * pCamera, char * szSkyCloud, char * szSkyGradient, float vTile)
{
	...
	if( szSkyCloud[0] )
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DSkyGradient::Init(), Can not load texture [%s]", szSkyCloud);
			return false;
		}
	}
	if( szSkyGradient[0] )
	{
		...
		{
			g_pA3DErrLog->ErrLog("A3DSkyGradient::Init(), Can not load texture [%s]", szSkyGradient);
			return false;
		}
	}
	...
	if( NULL == m_pSkyCloud )
	{
		g_pA3DErrLog->ErrLog("A3DSkyGradient::Init(), Not enough memory!");
		return false;
	}
	if( !m_pSkyCloud->Init(m_pA3DDevice, A3DVT_LVERTEX, m_nVertCount, m_nFaceCount * 3, 0) )
	{
		g_pA3DErrLog->ErrLog("A3DSkyGradient::Init(), Init the A3DStream fail!");
		return false;
	}
	m_pSkyGradient = new A3DStream();
	if( NULL == m_pSkyGradient )
	{
		g_pA3DErrLog->ErrLog("A3DSkyGradient::Init(), Not enough memory!");
		return false;
	}
	if( !m_pSkyGradient->Init(m_pA3DDevice, A3DVT_LVERTEX, m_nVertCount, m_nFaceCount * 3, 0) )
	{
		g_pA3DErrLog->ErrLog("A3DSkyGradient::Init(), Init the A3DStream fail!");
		return false;
	}
...
bool A3DSkyGradient::Render()
{
	...
	if( m_pCamera )
	{
		...
		m_WorldMatrix = Translate(vecPos.x, vecPos.y - m_vRadius * (float)cos(m_vPitchRange), vecPos.z);
	}
	...
	if( m_pTextureSkyGradient )
	{
		...
		m_pA3DDevice->GetD3DDevice()->SetRenderState(D3DRS_WRAP0, D3DWRAP_U);
		m_pA3DDevice->DrawIndexedPrimitive(A3DPT_TRIANGLELIST, 0, m_nVertCount, 0, m_nFaceCount);
		m_pA3DDevice->GetD3DDevice()->SetRenderState(D3DRS_WRAP0, 0);
...
bool A3DSkyGradient::SetSkyCloudMap(char * szCloudTexture)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DSkyGradient::Init(), Can not load texture [%s]", szCloudTexture);
		return false;
	}
...
bool A3DSkyGradient::SetSkyGradientMap(char * szGradientTexture)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DSkyGradient::Init(), Can not load texture [%s]", szGradientTexture);
		return false;
	}
