...
bool A3DRain::EmitParticles()
{
	...
	if( m_pHostCamera )
	{
		...
		UpdateParentTM(Translate(vecCamPos.x, vecCamPos.y, vecCamPos.z));
		vecEmitterCenter = vecCamPos + m_pHostCamera->GetDirH() * GetEmitterWidth() * 0.75f;
	}
	...
	for(n=0; n<nToNew; n++)
	{
		...
		vecDir.x = vecDir.x * 0.5f + 0.02f;
		vecDir.y = -1.0f;
		vecDir.z = vecDir.z * 0.5f - 0.02f;
...
bool A3DRain::MakeDead(LPVOID pParticle)
{
	...
	if( m_vEmitterWidth - vDis + A3DRandGenerator::RandUniformFloat(0.0f, m_vEmitterWidth) < m_vEmitterWidth )
		return true;
	...
	if( pA3DCDS )
	{
		...
		bool bHit = pA3DCDS->RayTrace(vecStart, vecEnd - vecStart, 1.0f, &rayTrace, NULL);
		if( bHit )
		{
			m_pSplash->AddSplash(rayTrace.vPoint, rayTrace.vNormal, rayTrace.objectType == TRACE_OBJECT_TERRAIN);
		}
	}
...
bool A3DRain::Init(A3DDevice * pA3DDevice, FLOAT vAmplitude)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DRain::Init(), call parent init fail!");
		return false;
	}
...
bool A3DRain::SetRainCurtainTexture(char * szTextureMap)
{
	...
	{
		g_pA3DErrLog->ErrLog("A3DRain::SetRainCurtainTexture(), fail!");
		return false;
	}
...
bool A3DRainSplash::AddSplash(const A3DVECTOR3& vecPos, const A3DVECTOR3& vecDir, bool bOnTerrain)
{
	...
	for(n=0; n<m_nSpawnNumber; n++)
	{
		...
		A3DMATRIX4 matEmitDir = RotateX(DEG2RAD(vOffAxis - GetOffAxis())) * RotateZ(DEG2RAD(vOffPlane)) * RotateX(DEG2RAD(GetOffAxis()));
