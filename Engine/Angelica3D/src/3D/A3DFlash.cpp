...
bool A3DFlash::Init(A3DDevice * pDevice, APerlinNoise1D * pNoise, float vSize, int nSteps, A3DCOLOR color)
{
	...
	if( m_nNumSegments <= 0 )
	{
		g_pA3DErrLog->ErrLog("A3DFlash::Init(), invalid segments number [%d] for this flash!", m_nNumSegments);
		return false;
	}

	if( m_nNumSegments > A3DFLASH_MAX_SEGEMENT )
	{
		g_pA3DErrLog->ErrLog("A3DFlash::Init(), Too many segments [%d] for this flash!", m_nNumSegments);
		return false;
	}

	m_pVecStrikePath = (A3DVECTOR3 *) malloc((m_nNumSegments + 1) * sizeof(A3DVECTOR3));
	if( NULL == m_pVecStrikePath )
	{
		g_pA3DErrLog->ErrLog("A3DFlash::Init(), Not enough memory!");
		return false;
	}

	m_pVertexBuffer = (A3DLVERTEX *) malloc(m_nNumSegments * 4 * sizeof(A3DLVERTEX));
	...
	m_pIndexBuffer = (WORD *) malloc(m_nNumSegments * 6 * sizeof(WORD));
...
bool A3DFlash::SetTextureMap(char * szTextureMap)
{
	...
	if( !m_pA3DDevice->GetA3DEngine()->GetA3DTextureMan()->LoadTextureFromFileInFolder(szTextureMap, "Gfx\\Textures", &m_pTexture) )
	{
		...
		g_pA3DErrLog->ErrLog("A3DFlash::SetTextureMap(), fail!");
		m_pTexture = NULL;
	}
...
bool A3DFlash::BuildStrikePath()
{
	...
	A3DMATRIX4 matCoord = TransformMatrix(vecDir, vecUp, m_vecStart);
	...
	FLOAT		vSeed = A3DRandGenerator::RandUniformFloat(0.0f, 1024.0f);
	for(int i=1; i<m_nNumSegments; i++)
	{
		...
		vecNewPos.z = g_pA3DMathUtility->COS(vPitch);
		vecNewPos.x = g_pA3DMathUtility->SIN(vPitch) * g_pA3DMathUtility->COS(vDeg);
		vecNewPos.y = g_pA3DMathUtility->SIN(vPitch) * g_pA3DMathUtility->SIN(vDeg);

		vLengthAxis += A3DRandGenerator::RandUniformFloat(vMinL, vMaxL); 
		vecNewPos = vecNewPos * (vLengthAxis / g_pA3DMathUtility->COS(vPitch));
