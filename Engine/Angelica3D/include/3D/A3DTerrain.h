...
#define A3DTERRAIN_MAX_TEXTURE				8
#define	A3DTERRAIN_MAXMARKVERT				32
#define A3DTERRAIN_MAXMARKINDEX				64
...
} TERRAINPARAM, * PTERRAINPARAM;

typedef struct _TERRAIN_MARKSPLIT
{
	A3DLVERTEX *	pVerts;
	WORD *			pIndices;
	int				nNumVert;
	int				nNumIndices;
} TERRAIN_MARKSPLIT, * PTERRAIN_MARKSPLIT;
...
private:
	...
	A3DStream			* m_pStreams[A3DTERRAIN_MAX_TEXTURE];
	A3DTexture			* m_pTextures[A3DTERRAIN_MAX_TEXTURE];

	FLOAT				m_vOptimizeStrength;

	//Normal vertex buffer and index buffers;
	A3DLVERTEX			* m_pVertexBuffers[A3DTERRAIN_MAX_TEXTURE];
	WORD				* m_pIndexBuffers[A3DTERRAIN_MAX_TEXTURE];
	int					m_nVisibleVerts[A3DTERRAIN_MAX_TEXTURE];
	int					m_nVisibleFaces[A3DTERRAIN_MAX_TEXTURE];

	int					m_nMaxVerts[A3DTERRAIN_MAX_TEXTURE];
	int					m_nMaxIndices[A3DTERRAIN_MAX_TEXTURE];
	int					m_nMaxExtraVerts[A3DTERRAIN_MAX_TEXTURE];
	int					m_nMaxExtraIndices[A3DTERRAIN_MAX_TEXTURE];

	//Extra vertex and index buffer which are used to add extra patch 
	//to avoid the seam between two cell;
	A3DLVERTEX			* m_pExtraVertexBuffers[A3DTERRAIN_MAX_TEXTURE];
	WORD				* m_pExtraIndexBuffers[A3DTERRAIN_MAX_TEXTURE];
	int					m_nExtraVisibleVerts[A3DTERRAIN_MAX_TEXTURE];
	int					m_nExtraVisibleFaces[A3DTERRAIN_MAX_TEXTURE];
	...
	int					m_nNumTexture;
	char				m_szTextureMap[A3DTERRAIN_MAX_TEXTURE][MAX_PATH];
...
public:
	...
	bool Init(A3DDevice * pA3DDevice, PTERRAINPARAM pTerrainParam, char * szTexture, char * szHeightMap, char * szTextureMap);
	bool Release();
...
public:
	...
	bool SplitMark(A3DAABB& aabb, A3DLVERTEX* aVerts, WORD* aIndices, bool bJog, int* piNumVert, int* piNumIdx);
	bool ClipInCell(A3DAABB& aabb, int nCellX, int nCellY, TERRAIN_MARKSPLIT * pMarkSplit);
...
public:
	...
	bool RayTrace(A3DVECTOR3& vecStart, A3DVECTOR3& vecVelocity, FLOAT vTime, RAYTRACE * pRayTrace);
	bool RayPickInCell(int x, int y, A3DVECTOR3& vecStart, A3DVECTOR3& vecDir, FLOAT vDis, RAYTRACE * pRayTrace);
	bool OBBTrace(OBBTRACE * pOBBTrace, A3DOBB& obb, OBBSHAPE& obbShape);
...
public:
	...
	inline int GetSightRange() { return m_nSightRange; }
...
public:
	...
	bool Create(A3DDevice * pA3DDevice, int nNumTexture, int nWidth, int nHeight, int nSightRange=80, float vCellSize=20.0f);
	...
	// Set and retrieve cell's texture index;
