...
enum A3DGFX_PLAYEVENT_TYPE
{
	A3DGFX_PLAYEVENT_TYPE_NULL = -1,
	A3DGFX_PLAYEVENT_TYPE_ITEM = 0,
	A3DGFX_PLAYEVENT_TYPE_RANDITEM = 1,
	A3DGFX_PLAYEVENT_TYPE_VOLUMETRICITEM = 2  // volumetric item is composed by several items that will shift forward or backward a little
};
...
typedef struct _A3DGFX_PLAYEVENT_ITEM
{
	A3DObject			* pElement;
	DWORD				dwClassID;
} A3DGFX_PLAYEVENT_ITEM, * PA3DGFX_PLAYEVENT_ITEM;

typedef struct _A3DGFX_PLAYEVENT_RANDITEM
{
	int				nNumElements;
	A3DObject		** ppElements;
	DWORD			* pClassIDs;
} A3DGFX_PLAYEVENT_RANDITEM, * PA3DGFX_PLAYEVENT_RANDITEM;
...
public:
	...
	inline A3DGFX_PLAYEVENT_TYPE GetType() { return m_Type; }
	inline void SetType(A3DGFX_PLAYEVENT_TYPE type) { m_Type = type; }

	inline LPVOID GetItemPtr()				{ return m_pItem; }
	inline void SetItemPtr(LPVOID pItem)	{ m_pItem = pItem; }
