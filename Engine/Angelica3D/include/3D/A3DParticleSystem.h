...
protected:
	...
	inline A3DCOLOR GetParticleColor(FLOAT vHeight)
	{
	...
	inline int GetNextParticleLife()
	{
		return (int)(m_nLife * RandFloat(1.0f, m_nLifeVariation * 1.0f / m_nLife));
	}
	inline int GetLongestParticleLife()
	{
		return (int)(m_nLife * (1.0f + m_nLifeVariation * 1.0f / m_nLife));
	}
	...
	FLOAT					m_vDivergence;
	inline FLOAT GetNextParticleSpeedValue()
	{
		return RandFloat(m_vSpeed, m_vSpeed * m_vSpeedVariation * 0.01f) * m_vSpeedScale;
	}

	// Particle Size Control;
	FLOAT					m_vSize;
	FLOAT					m_vSizeVariation; //%
	inline FLOAT GetNextParticleSize()
	{
		return RandFloat(m_vSize, m_vSize * m_vSizeVariation * 0.01f) * m_vScale;
	}
	...
	inline FLOAT GetNextParticleInheritInfluence()
	{
		return RandFloat(m_vInheritInfluence, m_vInheritInfluence * m_vInheritInfluenceVariation * 0.01f);
	}
	...
	inline FLOAT GetNextParticleTension()
	{
		return RandFloat(m_vMetaParticleTension, m_vMetaParticleTension * m_vMetaParticleTensionVariation * 0.01f);
	}
	...
	inline FLOAT GetNextParticleSpinPhase()
	{
		return RandFloat(m_vSpinPhase, m_vSpinPhase * m_vSpinPhaseVariation * 0.01f);
	}
	...
	inline A3DMATRIX4 GetNextParticleSpinVariation()
	{
		...
		return RotateX(DEG2RAD(vSpinX)) * RotateY(DEG2RAD(vSpinY)) * RotateZ(DEG2RAD(vSpinZ));
	}
	...
	inline FLOAT GetNextParticleBubbleAmplitude()
	{
		return RandFloat(m_vBubbleAmplitude, m_vBubbleAmplitude * m_vBubbleAmplitudeVariation * 0.01f);
	}

	int						m_nBubblePeriod;
	FLOAT					m_vBubblePeriodVariation; //%
	inline int	 GetNextParticleBubblePeriod()
	{
		return (int)(m_nBubblePeriod * RandFloat(1.0f, m_vBubblePeriodVariation * 0.01f));
	}

	FLOAT					m_vBubblePhase;
	FLOAT					m_vBubblePhaseVariation; //%
	inline FLOAT GetNextParticleBubblePhase()
	{
		return RandFloat(m_vBubblePhase, m_vBubblePhase * m_vBubblePhaseVariation * 0.01f);
	}
...
protected:
	...
	inline FLOAT RandFloat(FLOAT average, FLOAT variation) { return A3DRandGenerator::RandUniformFloat(average - variation, average + variation); }

	inline bool AddParticle(LPBYTE pParticle);
	inline bool DeleteParticle(int nIndex);

	virtual bool MakeDead(LPVOID pParticle) = 0;
	virtual bool EmitParticles() = 0;
	inline  bool FillCommonParticle(COMMON_PARTICLE * pCommonParticle); // Fill one particle's common parameter;

	virtual bool UpdateParticles();
	inline  bool UpdateCommonParticle(COMMON_PARTICLE * pCommonParticle); // Update one particle's common parameter;
...
public:
	...
	inline bool ChangeTextureMap(char * pszTextureMap);
	...
	inline void SetColorNum(int nColorNum) { m_nColorNum = min(nColorNum, A3DPARTICLESYSTEM_MAXCOLORMAPS); }
	...
	inline bool SetColor(int i, A3DCOLOR rgbColor)
	{
		...
		RGBToHSV(rgbColor, &m_pHSVColors[i]);
		return true;
	}
...
bool A3DParticleSystem::FillCommonParticle(COMMON_PARTICLE * pCommonParticle)
{
	...
	if( pCommonParticle->vBubbleAmplitude != 0.0f )
	{
		...
		A3DMATRIX4 mat = RotateAxis(pCommonParticle->vecPos, pCommonParticle->vecDir, RandFloat(A3D_PI, A3D_PI));
...
typedef A3DParticleSystem * PA3DParticleSystem;
