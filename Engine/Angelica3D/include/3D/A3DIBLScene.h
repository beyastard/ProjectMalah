...
typedef struct _A3DIBLSCENE_TEXTURE_RECORD
{
	...
	A3DMESH_PROP			prop;						// The mesh's property; not really used by IBL scene
...
} A3DIBLSCENE_TEXTURE_RECORD, * PA3DIBLSCENE_TEXTURE_RECORD;

typedef struct _A3DIBLSCENE_FACE_RECORD
{
	WORD					nTexID;						// The texture id of this face;
	int						nRefTicks;					// The ticks last time referenced this face (in Engine tick unit);

} A3DIBLSCENE_FACE_RECORD, * PA3DIBLSCENE_FACE_RECORD;
...
typedef struct _A3DIBLSCENE_OPTION
{
	int						nVertexType;
...
} A3DIBLSCENE_OPTION, *PA3DIBLSCENE_OPTION;
...
private:
	typedef struct _A3DIBLSCENE_SORTEDFACE
	{
		float							vDisToCam;
		A3DIBLSCENE_TEXTURE_RECORD		*pTexRecord;
		WORD							wIndexInTexVisible;
	} A3DIBLSCENE_SORTEDFACE;
...
protected:
	...
	A3DIBLSCENE_OPTION			m_Option;

	int							m_nNumFaces;		// Total faces of this scene;
	A3DIBLVERTEX *				m_pAllFaces;		// Faces buffer that consist of seperate vertex;
	A3DIBLSCENE_FACE_RECORD *	m_pFaceRecords;		// Buffer stores each face's texture id and reference info;
	A3DIBLSCENE_SORTEDFACE *	m_pSortedFaces;		// Buffer used for sorts;

	int							m_nNumVisibleFaces;	// Number of Visible Faces;

	int							m_nNumTextures;		// Texture number;
	A3DIBLSCENE_TEXTURE_RECORD *m_pTextureRecords;	// Texture list;
...
protected:
	A3DIBLSCENE_TEXTURE_RECORD * FindTextureRecord(char * szTextureName, char * szLightMapName, bool b2Sided, A3DMESH_PROP prop);
...
public:
	...
	bool AddTriFace(char * szTextureName, char * szLightMapName, bool b2Sided, A3DIBLVERTEX * v0, A3DIBLVERTEX * v1, A3DIBLVERTEX * v2);
	bool LoadAllMaps();
