...
typedef struct _BILLBOARD_KEYINFO
{
	FLOAT		vSizeX;
	FLOAT		vSizeY;
	FLOAT		vSizeZ;
...
public:
	...
	inline void SetTextureMap(char * szTextureMap) { strncpy(m_szTextureMap, szTextureMap, MAX_PATH); }
	...
	inline void SetTexRow(int nTexRow) { m_nTexRow = max(nTexRow, 1); }
	inline void SetTexCol(int nTexCol) { m_nTexCol = max(nTexCol, 1); }
	inline void SetTexInterval(int nTexInterval) { m_nTexInterval = max(nTexInterval, 1); }
